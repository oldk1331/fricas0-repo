...
(DEFUN |SHDP;lessThanRlex| (|v1| |v2| |low| |high| $)
  (PROG (#1=#:G136 |i| |n2| |n1|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ (LETT |n1| (|spadConstant| $ 10) . #2=(|SHDP;lessThanRlex|))
            (LETT |n2| (|spadConstant| $ 10) . #2#)
            (SEQ (LETT |i| |low| . #2#) G190 (COND ((> |i| |high|) (GO G191)))
                 (SEQ
                  (LETT |n1|
     ...
     (PROG (#2=#:G166)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |SplitHomogeneousDirectProduct;|) #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
...
(MAKEPROP '|SplitHomogeneousDirectProduct| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL (|DirectProduct| 6 8) (|local| |#1|)
              ...
              |SHDP;<;2$B;2| (|NonNegativeInteger|) (|List| 8) (|List| 21)
              ...
              (|Matrix| 8) (|Record| (|:| |mat| 41) (|:| |vec| 46))
              (|Union| 8 '#1#) (|String|) (|SingleInteger|) (|Vector| 8)
              (|List| 11))
           '#(|qelt| 28 < 34) 'NIL
           (CONS
            (|makeByteWordVec2| 37
