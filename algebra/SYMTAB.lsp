...
(DEFUN |SYMTAB;coerce;$T;2| (|t| $) |t|) 

(DEFUN |SYMTAB;symbolTable;L$;3| (|l| $) (SPADCALL |l| (QREFELT $ 14))) 

(DEFUN |SYMTAB;empty;$;4| ($) (SPADCALL (QREFELT $ 16))) 

(DEFUN |SYMTAB;parametersOf;$L;5| (|tab| $) (SPADCALL |tab| (QREFELT $ 19))) 

(DEFUN |SYMTAB;declare!;SFt$Ft;6| (|name| |type| |tab| $)
      ...
      (PROGN
       ...
       (SEQ (LETT |u| NIL . #3#)
            ...
            (SEQ
             (EXIT
              (COND
               ((SPADCALL (SPADCALL |u| |tab| (QREFELT $ 27)) (QREFELT $ 29))
                (LETT #2# (CONS |u| #2#) . #3#)))))
            (LETT #1# (CDR #1#) . #3#) (GO G190) G191
            (EXIT (NREVERSE #2#)))))))) 

(DEFUN |SYMTAB;typeList;Fst$L;10| (|type| |tab| $)
  (PROG (|arrayList| |scalarList| |uDim| #1=#:G148 |v| #2=#:G147 |sType|
         |uType| #3=#:G146 |u|)
    (RETURN
          ...
          (SEQ (LETT |u| NIL . #4#)
               ...
               (COND
                ((OR (ATOM #3#) (PROGN (LETT |u| (CAR #3#) . #4#) NIL))
                 (GO G191)))
               (SEQ (LETT |uType| (SPADCALL |u| |tab| (QREFELT $ 27)) . #4#)
...
(DEFUN |SYMTAB;typeList2| (|type| |tab| $)
  (PROG (|tl| |uDim| #1=#:G165 |v| #2=#:G164 |sType| |uType| #3=#:G163 |u|
         |symbolType|)
    (RETURN
     (SEQ (LETT |tl| NIL . #4=(|SYMTAB;typeList2|))
          ...
          (SEQ (LETT |u| NIL . #4#)
               ...
               (LETT #3# (CDR #3#) . #4#) (GO G190) G191 (EXIT NIL))
          (EXIT
           (COND ((NULL |tl|) |tl|) ('T (CONS (CONS 0 |symbolType|) |tl|)))))))) 

(DEFUN |SYMTAB;updateList| (|sType| |name| |lDims| |tl| $)
  (PROG (|entry| |l|)
    (RETURN
     (SEQ (LETT |l| (ASSOC |sType| |tl|) . #1=(|SYMTAB;updateList|))
          ...
          (EXIT
           (COND
            ...
            (#2#
             (SEQ (RPLACD |l| (CONS |entry| (SPADCALL |l| (QREFELT $ 44))))
                  (EXIT |tl|))))))))) 

(DEFUN |SYMTAB;newTypeLists;$Se;13| (|tab| $)
  (PROG (|tl| |lType| |lDims| #1=#:G179 |v| #2=#:G178 |dims| |sType| |uType|
         #3=#:G177 |u|)
    (RETURN
...
(DEFUN |SYMTAB;typeLists;$L;14| (|tab| $)
  (PROG (|tl| |types| #1=#:G186 |u| |fortranTypes|)
    (RETURN
     (SEQ
      (LETT |fortranTypes|
           ...
           ((QEQCAR |w| 1)
            (SPADCALL
             (PROG2 (LETT #1# |w| |SYMTAB;oForm2|)
                 (QCDR #1#)
               (|check_union| (QEQCAR #1# 1) (|Polynomial| (|Integer|)) #1#))
             (QREFELT $ 59))))))) 

(DEFUN |SYMTAB;oForm| (|v| $)
  (PROG (|ll| #1=#:G192 #2=#:G152 |uu| #3=#:G191)
    (RETURN
     (SEQ
      ...
      (PROGN
       ...
       (SEQ (LETT |u| NIL . #3#) (LETT #1# |t| . #3#) G190
            (COND
             ((OR (ATOM #1#) (PROGN (LETT |u| (CAR #1#) . #3#) NIL))
              (GO G191)))
            (SEQ (EXIT (LETT #2# (CONS (|SYMTAB;oForm| |u| $) #2#) . #3#)))
            (LETT #1# (CDR #1#) . #3#) (GO G190) G191
            (EXIT (NREVERSE #2#)))))))) 

(DEFUN |SYMTAB;printTypes;$V;18| (|tab| $)
  (PROG (|el| #1=#:G204 |u| #2=#:G203 |otl| |tl| #3=#:G202 |ty| |ft|)
    (RETURN
     (SEQ
      (LETT |ft|
      ...
      (LETT |el|
            (PROGN
             ...
             (SEQ (LETT |u| NIL . #4#)
                  ...
                  (SEQ
                   (EXIT
                    (LETT #2# (CONS (SPADCALL |u| (QREFELT $ 58)) #2#) . #4#)))
                  (LETT #1# (CDR #1#) . #4#) (GO G190) G191
                  (EXIT (NREVERSE #2#))))
            . #4#)
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1
                ...
                (LETT #1# T . #2#))
            (COND ((NOT #1#) (HREM |$ConstructorCache| '|SymbolTable|))))))))))) 
