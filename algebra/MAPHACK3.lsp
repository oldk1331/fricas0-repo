...
(DEFUN |MappingPackageInternalHacks3| (&REST #1=#:G106)
  (PROG ()
    (RETURN
     (PROG (#2=#:G107)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |MappingPackageInternalHacks3;|) #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
  ...
  (PROG (|pv$| $ |dv$| DV$3 DV$2 DV$1)
    (RETURN
     (PROGN
      ...
      (LETT |dv$| (LIST '|MappingPackageInternalHacks3| DV$1 DV$2 DV$3) . #1#)
      (LETT $ (GETREFV 12) . #1#)
      (QSETREFV $ 0 |dv$|)
