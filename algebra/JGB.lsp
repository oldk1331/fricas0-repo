...
(DEFUN |JGB;convert1| (|p| $)
  (PROG (|up| |res| |m| |clcup| |lcup| |x| |cc| |xx|)
    (RETURN
     (SEQ (LETT |xx| (SPADCALL |p| (QREFELT $ 12)) . #1=(|JGB;convert1|))
          (EXIT
           (COND
            ((QEQCAR |xx| 1)
             (SEQ (LETT |cc| (SPADCALL |p| (QREFELT $ 13)) . #1#)
            ...
            ('T
             (SEQ (LETT |x| (QCDR |xx|) . #1#)
                  ...
                  (LETT |up| (SPADCALL |p| |x| (QREFELT $ 20)) . #1#)
                  (LETT |res| (|spadConstant| $ 21) . #1#)
                  (SEQ G190
                       (COND
                        ((NULL
                          (COND ((SPADCALL |up| (QREFELT $ 23)) 'NIL) ('T 'T)))
                         (GO G191)))
                       (SEQ (LETT |lcup| (SPADCALL |up| (QREFELT $ 24)) . #1#)
                            (LETT |clcup| (|JGB;convert1| |lcup| $) . #1#)
                            (LETT |m|
                                  (SPADCALL |clcup| |x|
                            ...
                            (LETT |res|
                                  ...
                                  . #1#)
                            (EXIT
                             (LETT |up| (SPADCALL |up| (QREFELT $ 29)) . #1#)))
                       NIL (GO G190) G191 (EXIT NIL))
                  (EXIT |res|))))))))) 

(DEFUN |JGB;convert2| (|p| $)
  (PROG (|up| |res| |x| |cc| |xx|)
    (RETURN
            ...
            ('T
             (SEQ (LETT |x| (QCDR |xx|) . #1#)
                  ...
                  (SEQ G190
                       (COND
                        ((NULL
                          (COND ((SPADCALL |up| (QREFELT $ 36)) 'NIL) ('T 'T)))
                         (GO G191)))
                       (SEQ
                        (LETT |res|
                              (SPADCALL |res|
...
(DEFUN |JGB;groebner;2L;3| (|lp| $)
  (PROG (#1=#:G134 |dp| #2=#:G133 |gb| |nlp| #3=#:G132 |p| #4=#:G131)
    (RETURN
     (SEQ
      (LETT |nlp|
            (PROGN
             ...
             (SEQ (LETT |p| NIL . #5#) (LETT #3# |lp| . #5#) G190
                  (COND
                   ((OR (ATOM #3#) (PROGN (LETT |p| (CAR #3#) . #5#) NIL))
                    (GO G191)))
                  (SEQ
                   (EXIT (LETT #4# (CONS (|JGB;convert1| |p| $) #4#) . #5#)))
        ...
        (SEQ (LETT |dp| NIL . #5#) (LETT #1# |gb| . #5#) G190
             (COND
              ((OR (ATOM #1#) (PROGN (LETT |dp| (CAR #1#) . #5#) NIL))
               (GO G191)))
             (SEQ (EXIT (LETT #2# (CONS (|JGB;convert2| |dp| $) #2#) . #5#)))
             (LETT #1# (CDR #1#) . #5#) (GO G190) G191
             (EXIT (NREVERSE #2#))))))))) 

(DECLAIM (NOTINLINE |JetGroebner;|)) 

(DEFUN |JetGroebner| (&REST #1=#:G135)
  (PROG ()
    (RETURN
     (PROG (#2=#:G136)
       (RETURN
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |JetGroebner;|) #1#)
                (LETT #2# T . #3#))
            (COND ((NOT #2#) (HREM |$ConstructorCache| '|JetGroebner|))))))))))) 
