  ...
  (PROG (|locmap| |slist|)
    (RETURN
     (SEQ (LETT |locmap| |fn| . #1=(|SOLVEFOR;mapSolve;UPMR;1|))
          (SETELT $ 10 (IDENTITY |locmap|)) (SETELT $ 9 'T) (SETELT $ 8 NIL)
          (LETT |slist| (SPADCALL |p| (QREFELT $ 12)) . #1#) (SETELT $ 9 'NIL)
          (LETT |locmap| (LIST #'|SOLVEFOR;mapSolve;UPMR;1!0|) . #1#)
          (SETELT $ 10 (IDENTITY |locmap|))
          (EXIT (CONS |slist| (QREFELT $ 8))))))) 

(DEFUN |SOLVEFOR;mapSolve;UPMR;1!0| (|x| $$) (IDENTITY |x|)) 

(DEFUN |SOLVEFOR;part| (|s| $)
  (PROG (|t|)
    (RETURN
     (SEQ
      (COND
        ...
        (SEQ (LETT |t| (SPADCALL |s| (QREFELT $ 10)) |SOLVEFOR;part|)
             (EXIT
              (COND ((SPADCALL |t| |s| (QREFELT $ 18)) |s|)
                    (#1='T
                     (SEQ (SETELT $ 8 (CONS (CONS |t| |s|) (QREFELT $ 8)))
                          (EXIT |t|)))))))
       (#1# |s|)))))) 

(DEFUN |SOLVEFOR;intsolve| (|u| $)
  (PROG (|n|)
    (RETURN
     (SEQ
      (LETT |u|
            (QCAR
             (SPADCALL (SPADCALL (SPADCALL |u| (QREFELT $ 20)) (QREFELT $ 24))
                       1 (QREFELT $ 26)))
            . #1=(|SOLVEFOR;intsolve|))
     ...
     (SEQ (LETT |ls| NIL . #3=(|SOLVEFOR;solve;UPL;4|))
          (SEQ (LETT |f| NIL . #3#)
               ...
               (SEQ (LETT |lsf| (|SOLVEFOR;intsolve| (QCAR |f|) $) . #3#)
                    (EXIT
                     (SEQ (LETT |i| 1 . #3#) (LETT #1# (QCDR |f|) . #3#) G190
                          (COND ((|greater_SI| |i| #1#) (GO G191)))
                          (SEQ (EXIT (LETT |ls| (APPEND |lsf| |ls|) . #3#)))
                          (LETT |i| (|inc_SI| |i|) . #3#) (GO G190) G191
                          (EXIT NIL))))
               (LETT #2# (CDR #2#) . #3#) (GO G190) G191 (EXIT NIL))
          (EXIT |ls|))))) 

(DEFUN |SOLVEFOR;particularSolution;UPF;5| (|u| $)
  (PROG (|n|)
    (RETURN
     (SEQ
      (LETT |u|
...
(DEFUN |SOLVEFOR;needDegree| (|n| |u| $)
  (COND ((EQL (SPADCALL |u| (QREFELT $ 28)) |n|) 'T)
        ('T
         (|error|
          (|concat| "Polynomial must be of degree " (|::| |n| (|String|))))))) 

(DEFUN |SOLVEFOR;needLcoef| (|cn| $)
  (COND ((SPADCALL |cn| (|spadConstant| $ 33) (QREFELT $ 43)) 'T)
        ('T (|error| "Leading coefficient must not be 0.")))) 

(DEFUN |SOLVEFOR;needChar0| ($)
  (COND ((EQL (SPADCALL (QREFELT $ 44)) 0) 'T)
        ('T (|error| "Formula defined only for fields of characteristic 0.")))) 

(DEFUN |SOLVEFOR;linear;UPL;9| (|u| $)
  (SEQ (|SOLVEFOR;needDegree| 1 |u| $)
       (EXIT
        (SPADCALL (SPADCALL |u| 1 (QREFELT $ 31))
                  (SPADCALL |u| 0 (QREFELT $ 31)) (QREFELT $ 34))))) 

(DEFUN |SOLVEFOR;quadratic;UPL;10| (|u| $)
...
(DEFUN |SOLVEFOR;quartic;UPL;12| (|u| $)
...
(DEFUN |SOLVEFOR;equ| (|x| |y| $)
  (COND
   ((QEQCAR (SPADCALL (SPADCALL |x| |y| (QREFELT $ 49)) (QREFELT $ 51)) 1) 'T)
   ('T 'NIL))) 

(DEFUN |SOLVEFOR;linear;2FL;14| (|c1| |c0| $)
  (SEQ (|SOLVEFOR;needLcoef| |c1| $)
       (EXIT
        (LIST (SPADCALL (SPADCALL |c0| |c1| (QREFELT $ 52)) (QREFELT $ 53)))))) 

(DEFUN |SOLVEFOR;aLinear;3F;15| (|c1| |c0| $)
  (|SPADfirst| (SPADCALL |c1| |c0| (QREFELT $ 34)))) 

(DEFUN |SOLVEFOR;quadratic;3FL;16| (|c2| |c1| |c0| $)
  (PROG (D)
    (RETURN
          ...
          (EXIT
           (COND
            ((SPADCALL |c0| (|spadConstant| $ 33) (QREFELT $ 18))
             (CONS (|spadConstant| $ 33) (SPADCALL |c2| |c1| (QREFELT $ 34))))
            ((SPADCALL |c1| (|spadConstant| $ 33) (QREFELT $ 18))
             (LIST
              (SPADCALL
               (SPADCALL (SPADCALL |c0| |c2| (QREFELT $ 52)) (QREFELT $ 53))
            ...
            ('T
             (SEQ
                    ...
                    (SPADCALL
                     (|SOLVEFOR;part|
                      (SPADCALL (SPADCALL |c1| 2 (QREFELT $ 58))
                                (SPADCALL (SPADCALL 4 |c2| (QREFELT $ 59)) |c0|
                                          (QREFELT $ 60))
                                (QREFELT $ 49))
                      $)
                     (SPADCALL 1 2 (QREFELT $ 55)) (QREFELT $ 56))
              ...
              (EXIT
               (LIST
                (SPADCALL
                 (SPADCALL (SPADCALL |c1| (QREFELT $ 53)) D (QREFELT $ 61))
                 (SPADCALL 2 |c2| (QREFELT $ 59)) (QREFELT $ 52))
                (SPADCALL
                 (SPADCALL (SPADCALL |c1| (QREFELT $ 53)) D (QREFELT $ 49))
                 (SPADCALL 2 |c2| (QREFELT $ 59)) (QREFELT $ 52)))))))))))) 

(DEFUN |SOLVEFOR;aQuadratic;4F;17| (|c2| |c1| |c0| $)
  (PROG (D)
    (RETURN
     (SEQ (|SOLVEFOR;needLcoef| |c2| $) (|SOLVEFOR;needChar0| $)
          (EXIT
...
(DEFUN |SOLVEFOR;cubic;4FL;18| (|c3| |c2| |c1| |c0| $)
  (PROG (#1=#:G175 |u| #2=#:G174 S D R Q #3=#:G173 #4=#:G172 P #5=#:G171
         #6=#:G170 |a3| |a2| |a1|)
    (RETURN
                    ...
                    (SEQ (LETT |u| NIL . #8#)
                         ...
                         (SEQ
                          (EXIT
                           (LETT #4#
                                 (CONS
                                  (SPADCALL P (SPADCALL S |u| (QREFELT $ 60))
                                            (QREFELT $ 61))
                                  #4#)
                                 . #8#)))
                          ...
                          (SPADCALL
                           (SPADCALL
                            (SPADCALL (SPADCALL 9 |a1| (QREFELT $ 59)) |a2|
                                      (QREFELT $ 60))
                            (SPADCALL 27 |a3| (QREFELT $ 59)) (QREFELT $ 49))
                           ...
                           (LETT #2#
                                 (CONS
                                  (SPADCALL
                                   (SPADCALL P (SPADCALL S |u| (QREFELT $ 60))
                                             (QREFELT $ 61))
                                   (SPADCALL Q (SPADCALL S |u| (QREFELT $ 60))
                                             (QREFELT $ 52))
                                   (QREFELT $ 49))
                                  #2#)
                                 . #8#)))
...
(DEFUN |SOLVEFOR;aCubic;5F;19| (|c3| |c2| |c1| |c0| $)
  (PROG (S D R Q P |a3| |a2| |a1|)
    (RETURN
     (SEQ (|SOLVEFOR;needLcoef| |c3| $) (|SOLVEFOR;needChar0| $)
          (EXIT
           (COND
...
(DEFUN |SOLVEFOR;quartic;5FL;20| (|c4| |c3| |c2| |c1| |c0| $)
  (PROG (#1=#:G187 |s| #2=#:G186 |slist| |rt| |t0| |r| |q| |p| |a4| |a3| |a2|
         |a1|)
    (RETURN
             ...
             (SEQ
              ...
              (LETT |p|
                    (|SOLVEFOR;part|
                     (SPADCALL |a2|
                               (SPADCALL
                                (SPADCALL (SPADCALL 3 |a1| (QREFELT $ 59)) |a1|
                                          (QREFELT $ 60))
              ...
              (LETT |q|
                    (|SOLVEFOR;part|
                     (SPADCALL
                      (SPADCALL |a3|
                                (SPADCALL (SPADCALL |a1| |a2| (QREFELT $ 60))
              ...
              (LETT |r|
                    (|SOLVEFOR;part|
                     (SPADCALL
                      (SPADCALL
                       (SPADCALL |a4|
                                 (SPADCALL (SPADCALL |a1| |a3| (QREFELT $ 60))
              ...
              (COND
               ((|SOLVEFOR;equ| |q| (|spadConstant| $ 33) $)
                (LETT |t0|
                      (|SOLVEFOR;part|
                       (SPADCALL 2
                                 (SPADCALL |r| (SPADCALL 1 2 (QREFELT $ 55))
                                           (QREFELT $ 56))
                                 (QREFELT $ 59))
                       $)
                      . #4#))
                ...
                (LETT |t0|
                      (SPADCALL (|spadConstant| $ 30)
                                ...
                                (SPADCALL (SPADCALL 4 |r| (QREFELT $ 59))
                                          (QREFELT $ 53))
                                (SPADCALL
                                 (SPADCALL (SPADCALL 4 |p| (QREFELT $ 59)) |r|
                                           (QREFELT $ 60))
              ...
              (LETT |slist|
                    (APPEND
                     (SPADCALL (|spadConstant| $ 30) |rt|
                               (SPADCALL
                                (SPADCALL
                                 (SPADCALL (SPADCALL |q| |rt| (QREFELT $ 52))
                                           (QREFELT $ 53))
                                 |t0| (QREFELT $ 61))
...
(DEFUN |SOLVEFOR;aQuartic;6F;21| (|c4| |c3| |c2| |c1| |c0| $)
  (PROG (|s| |rt| |t0| |r| |q| |p| |a4| |a3| |a2| |a1|)
    (RETURN
     (SEQ (|SOLVEFOR;needLcoef| |c4| $) (|SOLVEFOR;needChar0| $)
          (EXIT
           (COND
        ...
        (COND
         ...
         ('T
          (UNWIND-PROTECT
            ...
            (COND
             ((NOT #2#)
              (HREM |$ConstructorCache| '|PolynomialSolveByFormulas|))))))))))) 
