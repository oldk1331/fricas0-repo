...
(DEFUN |PDECOMP;leftFactor;2UPU;1| (|f| |h| $)
  (PROG (|g| #1=#:G115 |r| |fr| |i|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ (LETT |g| (|spadConstant| $ 8) . #2=(|PDECOMP;leftFactor;2UPU;1|))
            (SEQ (LETT |i| 0 . #2#) G190
                 (COND
...
(DEFUN |PDECOMP;decompose;UP2NniU;2| (|f| |dg| |dh| $)
  (PROG (|g| |h| |df|)
    (RETURN
     (SEQ
      ...
      (EXIT
       (COND ((SPADCALL (* |dg| |dh|) |df| (QREFELT $ 24)) (CONS 1 "failed"))
             (#2='T
                      ...
                      (EXIT
                       (COND
                        ((QEQCAR |g| 0)
                         (PROGN
                          (LETT #1#
                                (APPEND (SPADCALL (QCDR |g|) (QREFELT $ 30))
...
(DEFUN |PDECOMP;rightFactorCandidate;UPNniUP;4| (|f| |dh| $)
  (PROG (|h| #1=#:G152 |c| #2=#:G151 #3=#:G150 |hdg| |k| |dg| |df|)
    (RETURN
     (SEQ
      ...
      (SEQ (LETT |k| 1 . #4#) G190 (COND ((|greater_SI| |k| |dh|) (GO G191)))
