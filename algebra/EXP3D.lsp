     ...
     (SEQ (LETT |faceIndexList| NIL . #2=(|EXP3D;faceIndex|))
          (SEQ (LETT |poly| NIL . #2#)
               ...
               (SEQ
                (EXIT
                 (LETT |faceIndexList|
                       (CONS (SPADCALL |poly| (QREFELT $ 10)) |faceIndexList|)
                       . #2#)))
               (LETT #1# (CDR #1#) . #2#) (GO G190) G191 (EXIT NIL))
          (EXIT (REVERSE |faceIndexList|)))))) 

(DEFUN |EXP3D;writePolygon| (|f1| |curves| $)
  (PROG (|s| #1=#:G118 |i| |faceIndexList| #2=#:G117 |curve|)
    (RETURN
     (SEQ (LETT |faceIndexList| NIL . #3=(|EXP3D;writePolygon|))
          (SEQ (LETT |curve| NIL . #3#) (LETT #2# |curves| . #3#) G190
               ...
               (SEQ
                (EXIT
                 (LETT |faceIndexList|
                       (APPEND |faceIndexList| (|EXP3D;faceIndex| |curve| $))
                       . #3#)))
...
(DEFUN |EXP3D;writeMesh| (|f1| |curves| $)
  (PROG (|s| #1=#:G127 |j| #2=#:G126 |i| |colLength| |rowLength|
         |meshIndexArray| #3=#:G125 |curve|)
    (RETURN
     (SEQ (LETT |meshIndexArray| NIL . #4=(|EXP3D;writeMesh|))
          (SEQ (LETT |curve| NIL . #4#) (LETT #3# |curves| . #4#) G190
               ...
               (SEQ
                (EXIT
                 (LETT |meshIndexArray|
                       (CONS (|EXP3D;faceIndex| |curve| $) |meshIndexArray|)
                       . #4#)))
          ...
          (EXIT
           (SEQ (LETT |i| 1 . #4#) (LETT #2# (- |rowLength| 1) . #4#) G190
                ...
                (SEQ
                 (EXIT
                  (SEQ (LETT |j| 1 . #4#) (LETT #1# (- |colLength| 1) . #4#)
    ...
    (RETURN
     (SEQ
      ...
      (SEQ (LETT |v| NIL . #3#) (LETT #2# |verts| . #3#) G190
           (COND
            ((OR (ATOM #2#) (PROGN (LETT |v| (CAR #2#) . #3#) NIL)) (GO G191)))
           (SEQ
            (EXIT
             (COND
