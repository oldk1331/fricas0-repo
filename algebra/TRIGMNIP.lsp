...
(DEFUN |TRIGMNIP;kcomplex| (|k| $)
  (PROG (#1=#:G162 |u| |a| #2=#:G159 |n| |arg| |op|)
    (RETURN
...
(DEFUN |TRIGMNIP;complexForm;FC;7| (|f| $)
  (PROG (|l|)
    (RETURN
     (COND
      ((NULL
...
(DEFUN |TRIGMNIP;locexplogs| (|f| $)
  (PROG (|g|)
    (RETURN
     (COND
      ((SPADCALL (CONS #'|TRIGMNIP;locexplogs!0| (VECTOR $ (QREFELT $ 8)))
                 (SPADCALL
...
(DEFUN |TRIGMNIP;complexNormalize;FSF;9| (|f| |x| $)
  (PROG (#1=#:G178 |g| |k| #2=#:G177)
    (RETURN
     (SEQ
      (COND
       ((SPADCALL
         (CONS #'|TRIGMNIP;complexNormalize;FSF;9!0| (VECTOR (QREFELT $ 8) $))
         (PROGN
          (LETT #2# NIL . #3=(|TRIGMNIP;complexNormalize;FSF;9|))
          (SEQ (LETT |k| NIL . #3#)
               (LETT #1#
                     (SPADCALL
                      (LETT |g| (SPADCALL |f| |x| (QREFELT $ 64)) . #3#)
                      (QREFELT $ 34))
                     . #3#)
               G190
               (COND
        ...
        (SPADCALL
         (QVELT
          (SPADCALL (|TRIGMNIP;localexplogs| |f| |g| $) |x| (QREFELT $ 74)) 0)
         (QREFELT $ 75)))
       ('T (QVELT (SPADCALL |g| |x| (QREFELT $ 77)) 0))))))) 

(DEFUN |TRIGMNIP;complexNormalize;FSF;9!0| (|y| $$)
  (PROG ($ RTRIG)
    (LETT $ (QREFELT $$ 1) . #1=(|TRIGMNIP;complexNormalize;FSF;9|))
    (LETT RTRIG (QREFELT $$ 0) . #1#)
    (RETURN
     (PROGN (SPADCALL (SPADCALL |y| (QREFELT $ 36)) RTRIG (QREFELT $ 57)))))) 

(DEFUN |TRIGMNIP;complexNormalize;2F;10| (|f| $)
  (PROG (|g| |h| |l|)
    (RETURN
     (SEQ
      (LETT |l|
      ...
      (EXIT
       (COND
        ((SPADCALL
          (CONS #'|TRIGMNIP;complexNormalize;2F;10!0| (VECTOR $ (QREFELT $ 8)))
...
(DEFUN |TRIGMNIP;complexNormalize;2F;10!0| (|x| $$)
  (PROG (RTRIG $)
...
(DEFUN |TRIGMNIP;complexElementary;FSF;11!0| (|y| $$)
  (PROG ($ RTRIG)
...
(DEFUN |TRIGMNIP;complexElementary;2F;12| (|f| $)
  (PROG (|g|)
    (RETURN
     (COND
      ((SPADCALL
        (CONS #'|TRIGMNIP;complexElementary;2F;12!0| (VECTOR $ (QREFELT $ 8)))
        (SPADCALL
         (LETT |g| (SPADCALL |f| (QREFELT $ 59))
...
(DEFUN |TRIGMNIP;complexElementary;2F;12!0| (|x| $$)
  (PROG (RTRIG $)
     ...
     (SEQ
      (SPADCALL (SPADCALL |g| (QREFELT $ 21))
                (PROGN
                 ...
                 (SEQ (LETT |k| NIL . #3#)
                      (LETT #1# (SPADCALL |f| (QREFELT $ 34)) . #3#) G190
                      (COND
                       ((OR (ATOM #1#) (PROGN (LETT |k| (CAR #1#) . #3#) NIL))
                        (GO G191)))
                      (SEQ
                       (EXIT
                        (COND
                         ((COND ((SPADCALL |k| '|tan| (QREFELT $ 38)) 'T)
                                ('T (SPADCALL |k| '|cot| (QREFELT $ 38))))
                          (LETT #2# (CONS (|TRIGMNIP;K2KG| |k| $) #2#)
                                . #3#)))))
                      (LETT #1# (CDR #1#) . #3#) (GO G190) G191
                      (EXIT (NREVERSE #2#))))
                (QREFELT $ 85)))))) 

(DEFUN |TRIGMNIP;trigs;2F;14| (|f| $)
  (PROG (|g|)
    (RETURN
     (SEQ
      (COND ((SPADCALL |f| (QREFELT $ 26)) |f|)
            ('T
             (SEQ
              (LETT |g| (SPADCALL (SPADCALL |f| (QREFELT $ 21)) (QREFELT $ 56))
                    |TRIGMNIP;trigs;2F;14|)
              (EXIT
               (SPADCALL (SPADCALL |g| (QREFELT $ 29))
                         (SPADCALL (QREFELT $ 12) (SPADCALL |g| (QREFELT $ 31))
                                   (QREFELT $ 14))
                         (QREFELT $ 52)))))))))) 

(DEFUN |TrigonometricManipulations| (&REST #1=#:G203)
  (PROG ()
    (RETURN
     (PROG (#2=#:G204)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
            ...
            (COND
             ((NOT #2#)
              (HREM |$ConstructorCache| '|TrigonometricManipulations|))))))))))) 
