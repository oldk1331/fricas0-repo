     ...
     (SEQ
      (PROGN
       ...
       (SEQ (LETT |y2| NIL . #4#) (LETT #2# (CDR |yl|) . #4#)
            ...
            (COND
             ((OR (ATOM #1#) (PROGN (LETT |y1| (CAR #1#) . #4#) NIL) (ATOM #2#)
                  (PROGN (LETT |y2| (CAR #2#) . #4#) NIL))
              (GO G191)))
            (SEQ
             (EXIT
              (LETT #3# (CONS (SPADCALL |y2| |y1| (QREFELT $ 7)) #3#) . #4#)))
      ...
      ('T
       (SPADCALL
        (SPADCALL
         ...
         (QREFELT $ 19))
        (SPADCALL
         (SPADCALL (QREFELT $ 13) (SPADCALL |k| (QREFELT $ 19)) (QREFELT $ 20))
         (|NEWTON;newtonAux| (SPADCALL |k| (|spadConstant| $ 8) (QREFELT $ 21))
          (SPADCALL |fact| |k| (QREFELT $ 22)) (|NEWTON;differences| |yl| $) $)
         (QREFELT $ 23))
        (QREFELT $ 24))))))) 
...
(DEFUN |NewtonInterpolation| (#1=#:G115)
  (PROG ()
    (RETURN
     (PROG (#2=#:G116)
       (RETURN
