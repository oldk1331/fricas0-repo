...
(DEFUN |COMPCODE;genCatHeader| (|shortName| |longName| $)
  (LIST
   (SPADCALL (LIST ")abbrev category " |shortName| " " |longName|)
             (QREFELT $ 8))
   "" (SPADCALL (LIST |longName| "() : Category == Type with") (QREFELT $ 8))
   "" "")) 

(DEFUN |COMPCODE;genFuncDeclar| (|content| |numb| $)
  (PROG (|resStr| |notfst| #1=#:G118 |thisTerm| |resType| |deduct| #2=#:G117
         |fac|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
        ...
        (LETT |resStr|
              (SPADCALL (LIST "  fn" (STRINGIMAGE |numb|) ":(") (QREFELT $ 8))
              . #3#)
        (LETT |notfst| 'NIL . #3#)
        (SEQ (LETT |thisTerm| NIL . #3#) (LETT #1# |fac| . #3#) G190
             (COND
              ((OR (ATOM #1#) (PROGN (LETT |thisTerm| (CAR #1#) . #3#) NIL))
               (GO G191)))
             (SEQ
              (COND (|notfst| (LETT |resStr| (STRCONC |resStr| ",") . #3#)))
              (LETT |resStr|
...
(DEFUN |COMPCODE;genCatFooter| ($) (LIST "" "@")) 

(DEFUN |COMPCODE;genPackageHeader| (|shortName| |longName| |catName| $)
  (LIST
...
(DEFUN |COMPCODE;genLambdaTerm| (|n| |boundL| $)
  (PROG (|s| |ch| |boundL2| |varName| #1=#:G131 |i|)
    (RETURN
     (SEQ
      (EXIT
                    ...
                    ((SPADCALL |i| 0 (QREFELT $ 19))
                     (COND
                      ((SPADCALL |i| (LENGTH |boundL|) (QREFELT $ 20))
                       (PROGN
                        (LETT #1# (SPADCALL |boundL| |i| (QREFELT $ 23)) . #2#)
                        (GO #1#))))))
                   (EXIT (LETT |s| (STRINGIMAGE (- |i| 1)) . #2#)))))
            (COND
             ((SPADCALL |n| (QREFELT $ 24))
              (LETT |s| (SPADCALL (SPADCALL |n| (QREFELT $ 26)) (QREFELT $ 27))
                    . #2#)))
            (COND
...
(DEFUN |COMPCODE;genFuncDefn| (|content| |numb| $)
  (PROG (#1=#:G135 |lhst| |lhsn| |lhs| |var| |sl|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |COMPCODE;genPackageFooter| ($) (LIST "" "@")) 

(DEFUN |COMPCODE;writeCategory;L3SV;8|
       (|content| |filename| |shortName| |longName| $)
  (PROG (#1=#:G143 |line| |lines| |fnNum| #2=#:G142 |thisEq| |f1|)
    (RETURN
     (SEQ
      (LETT |f1|
      ...
      (LETT |lines| (|COMPCODE;genCatHeader| |shortName| |longName| $) . #3#)
      (LETT |fnNum| 1 . #3#)
...
(DEFUN |COMPCODE;writePackage;L4SV;9|
  ...
  (PROG (#1=#:G150 |line| |lines| |fnNum| #2=#:G149 |thisEq| |f1|)
    (RETURN
     (SEQ
      (LETT |f1|
            (SPADCALL (SPADCALL |filename| (QREFELT $ 38)) "output"
      ...
      (SEQ (LETT |thisEq| NIL . #3#) (LETT #2# |content| . #3#) G190
           ...
           (SEQ
            (LETT |lines|
                  (SPADCALL |lines| (|COMPCODE;genFuncDefn| |thisEq| |fnNum| $)
                            (QREFELT $ 41))
                  . #3#)
            (EXIT (LETT |fnNum| (+ |fnNum| 1) . #3#)))
           (LETT #2# (CDR #2#) . #3#) (GO G190) G191 (EXIT NIL))
      (LETT |lines|
            (SPADCALL |lines| (|COMPCODE;genPackageFooter| $) (QREFELT $ 41))
            . #3#)
      (SEQ (LETT |line| NIL . #3#) (LETT #1# |lines| . #3#) G190
           (COND
            ((OR (ATOM #1#) (PROGN (LETT |line| (CAR #1#) . #3#) NIL))
