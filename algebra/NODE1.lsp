     ...
     (SEQ
      (LETT |lk|
            (LIST
             (SPADCALL
              (LETT |yx|
                    (SPADCALL |y| (SPADCALL |x| (QREFELT $ 14)) (QREFELT $ 16))
                    . #1=(|NODE1;solve;2FBoSU;1|))
              (QREFELT $ 18)))
            . #1#)
      (LETT |lv| (LIST (SPADCALL (QREFELT $ 13) (QREFELT $ 19))) . #1#)
      (LETT |mm| (SPADCALL |m| |lk| |lv| (QREFELT $ 22)) . #1#)
      (LETT |nn| (SPADCALL |n| |lk| |lv| (QREFELT $ 22)) . #1#)
      (LETT |d|
            (SPADCALL
             ...
             (QREFELT $ 27))
            . #1#)
      (LETT |mm| (SPADCALL |d| |mm| (QREFELT $ 28)) . #1#)
      (LETT |nn| (SPADCALL |d| |nn| (QREFELT $ 28)) . #1#)
      (LETT |u| (|NODE1;integratingFactor| |mm| |nn| (QREFELT $ 10) |x| $)
            . #1#)
      (EXIT
       (COND
        ((QEQCAR |u| 0)
         (SEQ (LETT |mu| (QCDR |u|) . #1#)
              (LETT |mm| (SPADCALL |mm| |mu| (QREFELT $ 28)) . #1#)
              (LETT |nn| (SPADCALL |nn| |mu| (QREFELT $ 28)) . #1#)
              (EXIT
               (CONS 0
                     (SPADCALL
         ...
         (SEQ
          (LETT |w|
                (|NODE1;checkBernoulli| |m| |n|
                 (LETT |k1| (|SPADfirst| |lk|) . #1#) $)
                . #1#)
          (EXIT
           (COND
            ((QEQCAR |w| 0) (|NODE1;solveBernoulli| (QCDR |w|) |y| |x| |yx| $))
            (#2#
             (SEQ (LETT |v| (|NODE1;checkRiccati| |m| |n| |k1| $) . #1#)
                  (EXIT
                   (COND
                    ((QEQCAR |v| 0)
...
(DEFUN |NODE1;integratingFactor| (|m| |n| |y| |x| $)
  (PROG (|f| |d|)
    (RETURN
     (COND
      ((SPADCALL
        (LETT |d|
              (SPADCALL (SPADCALL |m| |y| (QREFELT $ 31))
                        (SPADCALL |n| |x| (QREFELT $ 31)) (QREFELT $ 32))
              . #1=(|NODE1;integratingFactor|))
        (QREFELT $ 37))
       (CONS 0 (|spadConstant| $ 38)))
      ((NULL
        (SPADCALL |y|
                  (SPADCALL (LETT |f| (SPADCALL |d| |n| (QREFELT $ 39)) . #1#)
                            (QREFELT $ 41))
                  (QREFELT $ 42)))
           ...
           (EXIT
            (COND
             ((OR (< |d| 2)
                  (OR
                   (SPADCALL
                    (SPADCALL (LETT |pp| (SPADCALL |p| (QREFELT $ 52)) . #2#)
                              (QREFELT $ 51))
                    1 (QREFELT $ 53))
                   (OR
                    (OR
    ...
    (RETURN
     (SEQ (LETT |n1| (- 1 (QVELT |rec| 2)) . #2=(|NODE1;solveBernoulli|))
          (LETT |deq|
                (SPADCALL
                 (SPADCALL (SPADCALL |yx| |x| (QREFELT $ 31))
                           (SPADCALL
                            (SPADCALL |n1| (QVELT |rec| 0) (QREFELT $ 59)) |yx|
                            (QREFELT $ 28))
                           (QREFELT $ 33))
                 (SPADCALL |n1| (QVELT |rec| 1) (QREFELT $ 59)) (QREFELT $ 33))
                . #2#)
          (LETT |sol|
                (PROG2 (LETT #1# (SPADCALL |deq| |y| |x| (QREFELT $ 63)) . #2#)
                    (QCDR #1#)
          ...
          (EXIT
           (CONS 0
                 (SPADCALL
                  (SPADCALL (SPADCALL |yx| |n1| (QREFELT $ 64)) (QCAR |sol|)
                            (QREFELT $ 32))
                  (|SPADfirst| (QCDR |sol|)) (QREFELT $ 39)))))))) 

(DEFUN |NODE1;checkRiccati| (|m| |n| |ky| $)
  (PROG (|a2| |a1| |a0| |p| |y| |q| |f|)
    (RETURN
     (SEQ
      (LETT |q|
            (SPADCALL
           ...
           (OR
            (OR
             ...
             (NULL
              (SPADCALL
               (LETT |a0|
                     (SPADCALL (SPADCALL |p| 0 (QREFELT $ 68)) (QREFELT $ 27))
                     . #1#)
               |y| (QREFELT $ 45))))
            (NULL
             (SPADCALL
              (LETT |a1|
                    (SPADCALL (SPADCALL |p| 1 (QREFELT $ 68)) (QREFELT $ 27))
                    . #1#)
              |y| (QREFELT $ 45))))
           (NULL
            (SPADCALL
             (LETT |a2|
                   (SPADCALL (SPADCALL |p| 2 (QREFELT $ 68)) (QREFELT $ 27))
                   . #1#)
             |y| (QREFELT $ 45))))
...
(DEFUN |NODE1;partSolRiccati| (|l| |y| |x| |yx| $)
  (PROG (#1=#:G197 |bas| |gsol| |lineq| |q1| |q0| |l3|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
        ...
        (LETT |lineq|
              ...
              . #2#)
        (SEQ (LETT |gsol| (SPADCALL |lineq| |y| |x| (QREFELT $ 63)) . #2#)
             (EXIT
              (COND
               ((QEQCAR |gsol| 0)
                (COND
                 ((NULL (LETT |bas| (QCDR (QCDR |gsol|)) . #2#))
                  (PROGN (LETT #1# (CONS 1 #3="failed") . #2#) (GO #1#)))))
               ('T (PROGN (LETT #1# (CONS 1 #3#) . #2#) (GO #1#))))))
        (EXIT
         (CONS 0
...
(DEFUN |NonLinearFirstOrderODESolver| (&REST #1=#:G199)
  (PROG ()
    (RETURN
     (PROG (#2=#:G200)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |NonLinearFirstOrderODESolver;|) #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
