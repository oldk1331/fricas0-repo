
(/VERSIONCHECK 2) 

(DEFUN |STEXPST;expre| (|r| |e| |dx| $)
  (PROG ()
    (RETURN
     (SPADCALL |r| (CONS #'|STEXPST;expre!0| (VECTOR $ |dx| |e|))
               (QREFELT $ 11))))) 

(DEFUN |STEXPST;expre!0| ($$)
  (PROG (|e| |dx| $)
    (LETT |e| (QREFELT $$ 2) . #1=(|STEXPST;expre|))
    (LETT |dx| (QREFELT $$ 1) . #1#)
    (LETT $ (QREFELT $$ 0) . #1#)
    (RETURN (PROGN (SPADCALL |e| |dx| (QREFELT $ 9)))))) 

(DEFUN |STEXPST;exp;2S;2| (|z| $)
  (PROG (|coef|)
    (RETURN
     (SEQ
      (COND
       ((SPADCALL |z| (QREFELT $ 13))
        (SPADCALL (|spadConstant| $ 14) (SPADCALL (QREFELT $ 15))
                  (QREFELT $ 16)))
       (#1='T
        (SEQ (LETT |coef| (SPADCALL |z| (QREFELT $ 17)) |STEXPST;exp;2S;2|)
             (EXIT
              (COND
               ((SPADCALL |coef| (|spadConstant| $ 18) (QREFELT $ 19))
                (SPADCALL (CONS #'|STEXPST;exp;2S;2!0| (VECTOR |z| $))
                          (QREFELT $ 23)))
               (#1#
                (SPADCALL (CONS #'|STEXPST;exp;2S;2!1| (VECTOR |z| $ |coef|))
                          (QREFELT $ 23)))))))))))) 

(DEFUN |STEXPST;exp;2S;2!1| (|y| $$)
  (PROG (|coef| $ |z|)
    (LETT |coef| (QREFELT $$ 2) . #1=(|STEXPST;exp;2S;2|))
    (LETT $ (QREFELT $$ 1) . #1#)
    (LETT |z| (QREFELT $$ 0) . #1#)
    (RETURN
     (PROGN
      (|STEXPST;expre| (SPADCALL |coef| (QREFELT $ 24)) |y|
       (SPADCALL |z| (QREFELT $ 20)) $))))) 

(DEFUN |STEXPST;exp;2S;2!0| (|y| $$)
  (PROG ($ |z|)
    (LETT $ (QREFELT $$ 1) . #1=(|STEXPST;exp;2S;2|))
    (LETT |z| (QREFELT $$ 0) . #1#)
    (RETURN
     (PROGN
      (|STEXPST;expre| (|spadConstant| $ 14) |y| (SPADCALL |z| (QREFELT $ 20))
       $))))) 

(DEFUN |STEXPST;log;2S;3| (|z| $)
  (PROG (|c|)
    (RETURN
     (SEQ
      (COND
       ((SPADCALL |z| (QREFELT $ 13))
        (|error| "log: constant coefficient should not be 0"))
       (#1='T
        (SEQ (LETT |c| (SPADCALL |z| (QREFELT $ 17)) |STEXPST;log;2S;3|)
             (EXIT
              (COND
               ((SPADCALL |c| (|spadConstant| $ 18) (QREFELT $ 19))
                (|error| "log: constant coefficient should not be 0"))
               (#1#
                (SPADCALL (CONS #'|STEXPST;log;2S;3!1| (VECTOR |z| $ |c|))
                          (QREFELT $ 23)))))))))))) 

(DEFUN |STEXPST;log;2S;3!1| (|y| $$)
  (PROG (|c| $ |z|)
    (LETT |c| (QREFELT $$ 2) . #1=(|STEXPST;log;2S;3|))
    (LETT $ (QREFELT $$ 1) . #1#)
    (LETT |z| (QREFELT $$ 0) . #1#)
    (RETURN
     (PROGN
      (PROG ()
        (RETURN
         (SPADCALL (SPADCALL |c| (QREFELT $ 26))
                   (CONS #'|STEXPST;log;2S;3!0| (VECTOR |z| $ |y|))
                   (QREFELT $ 11)))))))) 

(DEFUN |STEXPST;log;2S;3!0| ($$)
  (PROG (|y| $ |z|)
    (LETT |y| (QREFELT $$ 2) NIL)
    (LETT $ (QREFELT $$ 1) NIL)
    (LETT |z| (QREFELT $$ 0) NIL)
    (RETURN
     (PROGN
      (SPADCALL (SPADCALL (SPADCALL |y| (QREFELT $ 28)) (QREFELT $ 25))
                (SPADCALL |z| (QREFELT $ 20)) (QREFELT $ 9)))))) 

(DECLAIM (NOTINLINE |StreamExponentialSeriesTranscendentalFunctions;|)) 

(DEFUN |StreamExponentialSeriesTranscendentalFunctions| (#1=#:G117)
  (PROG ()
    (RETURN
     (PROG (#2=#:G118)
       (RETURN
        (COND
         ((LETT #2#
                (|lassocShiftWithFunction| (LIST (|devaluate| #1#))
                                           (HGET |$ConstructorCache|
                                                 '|StreamExponentialSeriesTranscendentalFunctions|)
                                           '|domainEqualList|)
                . #3=(|StreamExponentialSeriesTranscendentalFunctions|))
          (|CDRwithIncrement| #2#))
         ('T
          (UNWIND-PROTECT
              (PROG1 (|StreamExponentialSeriesTranscendentalFunctions;| #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
              (HREM |$ConstructorCache|
                    '|StreamExponentialSeriesTranscendentalFunctions|))))))))))) 

(DEFUN |StreamExponentialSeriesTranscendentalFunctions;| (|#1|)
  (PROG (|pv$| $ |dv$| DV$1)
    (RETURN
     (PROGN
      (LETT DV$1 (|devaluate| |#1|)
            . #1=(|StreamExponentialSeriesTranscendentalFunctions|))
      (LETT |dv$| (LIST '|StreamExponentialSeriesTranscendentalFunctions| DV$1)
            . #1#)
      (LETT $ (GETREFV 30) . #1#)
      (QSETREFV $ 0 |dv$|)
      (QSETREFV $ 3 (LETT |pv$| (|buildPredVector| 0 0 NIL) . #1#))
      (|haddProp| |$ConstructorCache|
                  '|StreamExponentialSeriesTranscendentalFunctions| (LIST DV$1)
                  (CONS 1 $))
      (|stuffDomainSlots| $)
      (QSETREFV $ 6 |#1|)
      (SETF |pv$| (QREFELT $ 3))
      $)))) 

(MAKEPROP '|StreamExponentialSeriesTranscendentalFunctions| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL NIL (|local| |#1|) (|Stream| 6)
              (|StreamExponentialSeriesOperations| 6) (0 . *) (|Mapping| 7)
              (6 . |lazyIntegrate|) (|Boolean|) (12 . |empty?|) (17 . |One|)
              (21 . |empty|) (25 . |cons|) (31 . |frst|) (36 . |Zero|) (40 . =)
              (46 . |deriv|) (|Mapping| 7 7)
              (|ParadoxicalCombinatorsForStreams| 6) (51 . Y) (56 . |exp|)
              |STEXPST;exp;2S;2| (61 . |log|)
              (|StreamTaylorSeriesOperations| 6) (66 . -) |STEXPST;log;2S;3|)
           '#(|log| 71 |exp| 76) 'NIL
           (CONS (|makeByteWordVec2| 1 'NIL)
                 (CONS '#()
                       (CONS '#()
                             (|makeByteWordVec2| 29
                                                 '(2 8 7 7 7 9 2 8 7 6 10 11 1
                                                   7 12 0 13 0 6 0 14 0 7 0 15
                                                   2 7 0 6 0 16 1 7 6 0 17 0 6
                                                   0 18 2 6 12 0 0 19 1 8 7 7
                                                   20 1 22 7 21 23 1 6 0 0 24 1
                                                   6 0 0 26 1 27 7 7 28 1 0 7 7
                                                   29 1 0 7 7 25)))))
           '|lookupComplete|)) 
