    ...
    (RETURN
     (SEQ
      (LETT |coefs| (MAKEARR1 (LENGTH |vs|) (|spadConstant| $ 11))
            . #2=(|LSPP;poly2vect|))
      (SEQ (LETT |i| 1 . #2#) (LETT |v| NIL . #2#) (LETT #1# |vs| . #2#) G190
           (COND
            ((OR (ATOM #1#) (PROGN (LETT |v| (CAR #1#) . #2#) NIL)
                 (NULL (SPADCALL |p| (|spadConstant| $ 14) (QREFELT $ 16))))
             (GO G191)))
...
(DEFUN |LSPP;linSolve;LLR;3| (|ps| |vs| $)
  (PROG (|r|)
    (RETURN
     ...
     (PROG (#2=#:G125)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |LinearSystemPolynomialPackage;|) #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
