...
(DEFUN |MMLFORM;exprex;OfS;7| (|expr| $)
  (PROG (|s| |s1| #1=#:G189 |a| |nargs| |args| |sop| |op| |le|)
    (RETURN
     (SEQ
...
(DEFUN |MMLFORM;formatSpecial| (|op| |args| |prec| $)
  (PROG (|tmp| |base| |tmp3| |tmp2| |tmp1| |n| #1=#:G268 |u| #2=#:G267
         |prescript|)
    (RETURN
...
(DEFUN |MMLFORM;formatPrime| (|args| |prec| $)
  (PROG (|arg2| |s| |commaTest| #1=#:G276 |i| |commaS|)
    (RETURN
     (SEQ
...
(DEFUN |MMLFORM;formatNaryNoGroup| (|op| |sep| |opprec| |args| |prec| $)
  (PROG (|s| |l| #1=#:G314 |a| |opPrec| |ops| |p|)
    (RETURN
     (SEQ
      (COND ((NULL |args|) "")
            (#2='T
