          ...
          (SEQ (LETT |p| NIL . #2#) (LETT #1# |lp| . #2#) G190
               (COND
                ((OR (ATOM #1#) (PROGN (LETT |p| (CAR #1#) . #2#) NIL))
                 (GO G191)))
               (SEQ
                (EXIT (LETT |ts| (SPADCALL |p| |ts| (QREFELT $ 19)) . #2#)))
               (LETT #1# (CDR #1#) . #2#) (GO G190) G191 (EXIT NIL))
          (EXIT |ts|))))) 

(DEFUN |LAZM3PK;squareFree| (|ts| $)
  (PROG (|toSee| |toSave| |newus| #1=#:G130 |pwt| |lpwt| |us| #2=#:G129 |p|
         |newts| |lp|)
    (RETURN
     (SEQ
      (COND ((SPADCALL |ts| (QREFELT $ 20)) (LIST (SPADCALL (QREFELT $ 21))))
            ('T
             (SEQ
...
(DEFUN |LAZM3PK;normalizeIfCan;2ST;3| (|ts| $)
  (PROG (|newts| |p| |lp| #1=#:G135 #2=#:G142 #3=#:G143 |v| |lv|)
    (RETURN
     (SEQ
      (EXIT
                    ...
                    (SEQ (LETT |p| (|SPADfirst| |lp|) . #4#)
                         ...
                         (SEQ (LETT |v| NIL . #4#) (LETT #3# |lv| . #4#) G190
                              ...
                              (SEQ
                               (EXIT
                                (COND
                                 ((SPADCALL |v| (SPADCALL |p| (QREFELT $ 33))
                                            (QREFELT $ 34))
                                  "leave")
                                 ('T
                                  (SEQ
                              ...
                              (EXIT NIL))
                         (LETT |lp| (CDR |lp|) . #4#)
                         (LETT |p| (SPADCALL |p| |newts| (QREFELT $ 38)) . #4#)
                         (EXIT
...
(DEFUN |LAZM3PK;zeroSetSplit;LBL;4| (|lp| |clos?| $)
  (PROG (#1=#:G150 |ts| #2=#:G149 |toSave| #3=#:G148 |toSee|)
    (RETURN
     (SEQ
