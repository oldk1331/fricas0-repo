
(SDEFUN |FGLMICPK;zeroDim?|
        ((|lq2|
          (|List| (|HomogeneousDistributedMultivariatePolynomial| |ls| R)))
         (% (|Boolean|)))
        (SPROG
         ((#1=#:G19 NIL) (|s| NIL) (#2=#:G20 NIL) (|q2| NIL) (#3=#:G21 NIL)
          (|m| (|HomogeneousDistributedMultivariatePolynomial| |ls| R))
          (#4=#:G8 NIL) (|x| (|OrderedVariableList| |ls|))
          (|lv| (|List| (|OrderedVariableList| |ls|))))
         (SEQ (LETT |lq2| (SPADCALL |lq2| (QREFELT % 10)))
              (EXIT
               (COND
                ((OR (NULL |lq2|) (< (LENGTH |lq2|) (LENGTH (QREFELT % 7))))
                 NIL)
                ('T
                 (SEQ
                  (LETT |lv|
                        (PROGN
                         (LETT #1# NIL)
                         (SEQ (LETT |s| NIL) (LETT #2# (QREFELT % 7)) G190
                              (COND
                               ((OR (ATOM #2#)
                                    (PROGN (LETT |s| (CAR #2#)) NIL))
                                (GO G191)))
                              (SEQ
                               (EXIT
                                (LETT #1#
                                      (CONS
                                       (PROG2
                                           (LETT #4#
                                                 (SPADCALL |s| (QREFELT % 14)))
                                           (QCDR #4#)
                                         (|check_union2| (QEQCAR #4# 0)
                                                         (|OrderedVariableList|
                                                          (QREFELT % 7))
                                                         (|Union|
                                                          (|OrderedVariableList|
                                                           (QREFELT % 7))
                                                          #5="failed")
                                                         #4#))
                                       #1#))))
                              (LETT #2# (CDR #2#)) (GO G190) G191
                              (EXIT (NREVERSE #1#)))))
                  (SEQ (LETT |q2| NIL) (LETT #3# |lq2|) G190
                       (COND
                        ((OR (ATOM #3#) (PROGN (LETT |q2| (CAR #3#)) NIL)
                             (NULL (NULL (NULL |lv|))))
                         (GO G191)))
                       (SEQ (LETT |m| (SPADCALL |q2| (QREFELT % 16)))
                            (LETT |x|
                                  (PROG2
                                      (LETT #4# (SPADCALL |m| (QREFELT % 18)))
                                      (QCDR #4#)
                                    (|check_union2| (QEQCAR #4# 0)
                                                    (|OrderedVariableList|
                                                     (QREFELT % 7))
                                                    (|Union|
                                                     (|OrderedVariableList|
                                                      (QREFELT % 7))
                                                     #5#)
                                                    #4#)))
                            (EXIT
                             (COND
                              ((SPADCALL
                                (SPADCALL (SPADCALL |m| |x| (QREFELT % 20))
                                          (QREFELT % 22))
                                (QREFELT % 24))
                               (LETT |lv|
                                     (SPADCALL |x| |lv| (QREFELT % 26)))))))
                       (LETT #3# (CDR #3#)) (GO G190) G191 (EXIT NIL))
                  (EXIT (NULL |lv|))))))))) 

(SDEFUN |FGLMICPK;zeroDimensional?;LB;2|
        ((|lq1| (|List| (|Polynomial| R))) (% (|Boolean|)))
        (SPROG
         ((#1=#:G26 NIL) (|q1| NIL) (#2=#:G27 NIL)
          (|lq2|
           (|List| (|HomogeneousDistributedMultivariatePolynomial| |ls| R))))
         (SEQ
          (LETT |lq2|
                (PROGN
                 (LETT #1# NIL)
                 (SEQ (LETT |q1| NIL) (LETT #2# |lq1|) G190
                      (COND
                       ((OR (ATOM #2#) (PROGN (LETT |q1| (CAR #2#)) NIL))
                        (GO G191)))
                      (SEQ
                       (EXIT
                        (LETT #1# (CONS (SPADCALL |q1| (QREFELT % 29)) #1#))))
                      (LETT #2# (CDR #2#)) (GO G190) G191
                      (EXIT (NREVERSE #1#)))))
          (EXIT (|FGLMICPK;zeroDim?| |lq2| %))))) 

(SDEFUN |FGLMICPK;fglmIfCan;LU;3|
        ((|lq1| (|List| (|Polynomial| R)))
         (% (|Union| (|List| (|Polynomial| R)) "failed")))
        (SPROG
         ((#1=#:G41 NIL) (|q1| NIL) (#2=#:G42 NIL)
          (|lq2|
           (|List| (|HomogeneousDistributedMultivariatePolynomial| |ls| R)))
          (|lq3| (|List| (|DistributedMultivariatePolynomial| |ls| R)))
          (#3=#:G43 NIL) (|q3| NIL) (#4=#:G44 NIL))
         (SEQ
          (LETT |lq2|
                (PROGN
                 (LETT #1# NIL)
                 (SEQ (LETT |q1| NIL) (LETT #2# |lq1|) G190
                      (COND
                       ((OR (ATOM #2#) (PROGN (LETT |q1| (CAR #2#)) NIL))
                        (GO G191)))
                      (SEQ
                       (EXIT
                        (LETT #1# (CONS (SPADCALL |q1| (QREFELT % 29)) #1#))))
                      (LETT #2# (CDR #2#)) (GO G190) G191
                      (EXIT (NREVERSE #1#)))))
          (LETT |lq2| (SPADCALL |lq2| (QREFELT % 10)))
          (COND
           ((NULL (|FGLMICPK;zeroDim?| |lq2| %)) (EXIT (CONS 1 "failed"))))
          (LETT |lq3| (SPADCALL |lq2| (QREFELT % 34)))
          (LETT |lq1|
                (PROGN
                 (LETT #3# NIL)
                 (SEQ (LETT |q3| NIL) (LETT #4# |lq3|) G190
                      (COND
                       ((OR (ATOM #4#) (PROGN (LETT |q3| (CAR #4#)) NIL))
                        (GO G191)))
                      (SEQ
                       (EXIT
                        (LETT #3# (CONS (SPADCALL |q3| (QREFELT % 36)) #3#))))
                      (LETT #4# (CDR #4#)) (GO G190) G191
                      (EXIT (NREVERSE #3#)))))
          (EXIT (CONS 0 |lq1|))))) 

(SDEFUN |FGLMICPK;groebner;2L;4|
        ((|lq1| (|List| (|Polynomial| R))) (% (|List| (|Polynomial| R))))
        (SPROG
         ((#1=#:G57 NIL) (#2=#:G58 NIL)
          (|lq2|
           (|List| (|HomogeneousDistributedMultivariatePolynomial| |ls| R)))
          (#3=#:G59 NIL) (|q1| NIL) (#4=#:G60 NIL) (#5=#:G61 NIL)
          (#6=#:G62 NIL)
          (|lq3| (|List| (|DistributedMultivariatePolynomial| |ls| R)))
          (#7=#:G63 NIL) (|q3| NIL) (#8=#:G64 NIL))
         (SEQ
          (LETT |lq2|
                (PROGN
                 (LETT #1# NIL)
                 (SEQ (LETT |q1| NIL) (LETT #2# |lq1|) G190
                      (COND
                       ((OR (ATOM #2#) (PROGN (LETT |q1| (CAR #2#)) NIL))
                        (GO G191)))
                      (SEQ
                       (EXIT
                        (LETT #1# (CONS (SPADCALL |q1| (QREFELT % 29)) #1#))))
                      (LETT #2# (CDR #2#)) (GO G190) G191
                      (EXIT (NREVERSE #1#)))))
          (LETT |lq2| (SPADCALL |lq2| (QREFELT % 10)))
          (COND
           ((NULL (|FGLMICPK;zeroDim?| |lq2| %))
            (EXIT
             (SEQ
              (LETT |lq3|
                    (PROGN
                     (LETT #3# NIL)
                     (SEQ (LETT |q1| NIL) (LETT #4# |lq1|) G190
                          (COND
                           ((OR (ATOM #4#) (PROGN (LETT |q1| (CAR #4#)) NIL))
                            (GO G191)))
                          (SEQ
                           (EXIT
                            (LETT #3#
                                  (CONS (SPADCALL |q1| (QREFELT % 39)) #3#))))
                          (LETT #4# (CDR #4#)) (GO G190) G191
                          (EXIT (NREVERSE #3#)))))
              (LETT |lq3| (SPADCALL |lq3| (QREFELT % 41)))
              (EXIT
               (PROGN
                (LETT #5# NIL)
                (SEQ (LETT |q3| NIL) (LETT #6# |lq3|) G190
                     (COND
                      ((OR (ATOM #6#) (PROGN (LETT |q3| (CAR #6#)) NIL))
                       (GO G191)))
                     (SEQ
                      (EXIT
                       (LETT #5# (CONS (SPADCALL |q3| (QREFELT % 36)) #5#))))
                     (LETT #6# (CDR #6#)) (GO G190) G191
                     (EXIT (NREVERSE #5#)))))))))
          (LETT |lq3| (SPADCALL |lq2| (QREFELT % 34)))
          (EXIT
           (PROGN
            (LETT #7# NIL)
            (SEQ (LETT |q3| NIL) (LETT #8# |lq3|) G190
                 (COND
                  ((OR (ATOM #8#) (PROGN (LETT |q3| (CAR #8#)) NIL))
                   (GO G191)))
                 (SEQ
                  (EXIT (LETT #7# (CONS (SPADCALL |q3| (QREFELT % 36)) #7#))))
                 (LETT #8# (CDR #8#)) (GO G190) G191 (EXIT (NREVERSE #7#)))))))) 

(DECLAIM (NOTINLINE |FGLMIfCanPackage;|)) 

(DEFUN |FGLMIfCanPackage;| (|#1| |#2|)
  (SPROG ((DV$1 NIL) (DV$2 NIL) (|dv$| NIL) (% NIL) (|pv$| NIL))
         (PROGN
          (LETT DV$1 (|devaluate| |#1|))
          (LETT DV$2 |#2|)
          (LETT |dv$| (LIST '|FGLMIfCanPackage| DV$1 DV$2))
          (LETT % (GETREFV 43))
          (QSETREFV % 0 |dv$|)
          (QSETREFV % 3 (LETT |pv$| (|buildPredVector| 0 0 NIL)))
          (|haddProp| |$ConstructorCache| '|FGLMIfCanPackage| (LIST DV$1 DV$2)
                      (CONS 1 %))
          (|stuffDomainSlots| %)
          (QSETREFV % 6 |#1|)
          (QSETREFV % 7 |#2|)
          (SETF |pv$| (QREFELT % 3))
          %))) 

(DEFUN |FGLMIfCanPackage| (&REST #1=#:G65)
  (SPROG NIL
         (PROG (#2=#:G66)
           (RETURN
            (COND
             ((LETT #2#
                    (|lassocShiftWithFunction| (|devaluate_sig| #1# '(T NIL))
                                               (HGET |$ConstructorCache|
                                                     '|FGLMIfCanPackage|)
                                               '|domainEqualList|))
              (|CDRwithIncrement| #2#))
             ('T
              (UNWIND-PROTECT
                  (PROG1 (APPLY (|function| |FGLMIfCanPackage;|) #1#)
                    (LETT #2# T))
                (COND
                 ((NOT #2#)
                  (HREM |$ConstructorCache| '|FGLMIfCanPackage|)))))))))) 

(MAKEPROP '|FGLMIfCanPackage| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL NIL (|local| |#1|) (|local| |#2|) (|List| 15)
              (|GroebnerPackage| 6
                                 (|HomogeneousDirectProduct|
                                  (NRTEVAL (LENGTH (QREFELT % 7)))
                                  (|NonNegativeInteger|))
                                 15)
              (0 . |groebner|) (|Union| % '"failed") (|Symbol|)
              (|OrderedVariableList| (NRTEVAL (QREFELT % 7))) (5 . |variable|)
              (|HomogeneousDistributedMultivariatePolynomial|
               (NRTEVAL (QREFELT % 7)) 6)
              (10 . |leadingMonomial|) (|Union| 13 '"failed")
              (15 . |mainVariable|) (|SparseUnivariatePolynomial| %)
              (20 . |univariate|) (|SparseUnivariatePolynomial| 15)
              (26 . |leadingCoefficient|) (|Boolean|) (31 . |ground?|)
              (|List| 13) (36 . |remove|) (|Polynomial| 6)
              (|PolToPol| (NRTEVAL (QREFELT % 7)) 6) (42 . |pToHdmp|)
              (|List| 27) |FGLMICPK;zeroDimensional?;LB;2| (|List| 35)
              (|LinGroebnerPackage| (NRTEVAL (QREFELT % 7)) 6) (47 . |totolex|)
              (|DistributedMultivariatePolynomial| (NRTEVAL (QREFELT % 7)) 6)
              (52 . |dmpToP|) (|Union| 30 '"failed") |FGLMICPK;fglmIfCan;LU;3|
              (57 . |pToDmp|)
              (|GroebnerPackage| 6
                                 (|DirectProduct|
                                  (NRTEVAL (LENGTH (QREFELT % 7)))
                                  (|NonNegativeInteger|))
                                 35)
              (62 . |groebner|) |FGLMICPK;groebner;2L;4|)
           '#(|zeroDimensional?| 67 |groebner| 72 |fglmIfCan| 77) 'NIL
           (CONS (|makeByteWordVec2| 1 '(0))
                 (CONS '#(NIL)
                       (CONS
                        '#((|Join|
                            (|mkCategory|
                             (LIST
                              '((|zeroDimensional?|
                                 ((|Boolean|) (|List| (|Polynomial| |#1|))))
                                T)
                              '((|fglmIfCan|
                                 ((|Union| (|List| (|Polynomial| |#1|))
                                           "failed")
                                  (|List| (|Polynomial| |#1|))))
                                T)
                              '((|groebner|
                                 ((|List| (|Polynomial| |#1|))
                                  (|List| (|Polynomial| |#1|))))
                                T))
                             (LIST) NIL NIL)))
                        (|makeByteWordVec2| 42
                                            '(1 9 8 8 10 1 13 11 12 14 1 15 0 0
                                              16 1 15 17 0 18 2 15 19 0 13 20 1
                                              21 15 0 22 1 15 23 0 24 2 25 0 13
                                              0 26 1 28 15 27 29 1 33 32 8 34 1
                                              28 27 35 36 1 28 35 27 39 1 40 32
                                              32 41 1 0 23 30 31 1 0 30 30 42 1
                                              0 37 30 38)))))
           '|lookupComplete|)) 
