...
(DEFUN |COMBINAT;partition;2I;1| (|n| $)
  (PROG (|s| |t| #1=#:G144 |u| |l| |k| #2=#:G151 |i| #3=#:G140 |m|)
    (RETURN
     (SEQ
      (COND
       ((SPADCALL (SPADCALL (QREFELT $ 18) (QREFELT $ 20)) 0 (QREFELT $ 22))
                       ...
                       (SEQ
                        (EXIT
                         (SEQ (LETT |k| 1 . #5#) G190 NIL
                              ...
                              (EXIT NIL)))
                        #1# (EXIT #1#))
                       (EXIT (SPADCALL (QREFELT $ 18) |i| |t| (QREFELT $ 35))))
      ...
      (COND
       ...
       (#2='T
        (SEQ
         ...
         (SEQ
          (LETT |k|
                (SPADCALL (SPADCALL |s| (|spadConstant| $ 8) (QREFELT $ 28))
                          (QREFELT $ 26))
                . #3#)
...
(DEFUN |COMBINAT;binomial;3I;3| (|n| |m| $)
  (PROG (|b| #1=#:G169 |k| |s| |#G17| |#G16| #2=#:G168 |#G15| |#G14|)
    (RETURN
     (SEQ
      (EXIT
       (COND
        ...
        ('T
         (COND
          ...
          (#3='T
           (SEQ
            (PROGN
             (LETT |#G14| (|spadConstant| $ 7) . #4=(|COMBINAT;binomial;3I;3|))
              ...
              (EXIT
               (LETT |b|
                     (SPADCALL
                      (SPADCALL |b|
                                (SPADCALL
                                 (SPADCALL |n| (SPADCALL |k| (QREFELT $ 25))
                                           (QREFELT $ 42))
...
(DEFUN |COMBINAT;multinomial;ILI;4| (|n| |m| $)
  (PROG (|s| #1=#:G180 |t| #2=#:G172 #3=#:G171 #4=#:G173 #5=#:G179 #6=#:G125
         #7=#:G170 #8=#:G177 #9=#:G178)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
          ...
          (#11#
           (SEQ (LETT |s| (|spadConstant| $ 8) . #10#)
                (SEQ (LETT |t| NIL . #10#) (LETT #1# |m| . #10#) G190
                     (COND
                      ((OR (ATOM #1#) (PROGN (LETT |t| (CAR #1#) . #10#) NIL))
                       (GO G191)))
                     (SEQ
                      (EXIT
                       (LETT |s|
      ...
      (COND
       ...
       ('T
        (SEQ
         ...
         (SEQ
          (LETT |k|
                (SPADCALL (SPADCALL |m| (|spadConstant| $ 8) (QREFELT $ 28))
                          (QREFELT $ 26))
                . #2#)
              ...
              ('T
               (SEQ
                ...
                (SEQ (LETT |k| 1 . #4#)
                     (LETT #2#
                           (SPADCALL
                            (SPADCALL |n| (|spadConstant| $ 8) (QREFELT $ 42))
                            (QREFELT $ 26))
                           . #4#)
                ...
                (EXIT
                 (SPADCALL (QCDR (QREFELT $ 13))
                           (PROG1
                               (LETT #1# (SPADCALL |m| (QREFELT $ 26)) . #4#)
                             (|check_subtype| (>= #1# 0)
                                              '(|NonNegativeInteger|) #1#))
                           (QREFELT $ 49)))))))))))) 

(DEFUN |COMBINAT;stirling2;3I;7| (|n| |m| $)
  (PROG (|t| #1=#:G196 |s| #2=#:G201 |k|)
    (RETURN
     (SEQ
      (COND
       ((OR
       ...
       ('T
        (SEQ
         ...
         (SEQ (LETT |k| 1 . #3#) (LETT #2# (SPADCALL |m| (QREFELT $ 26)) . #3#)
              G190 (COND ((|greater_SI| |k| #2#) (GO G191)))
  ...
  (PROG (#1=#:G203)
    (RETURN
     (PROG2
         (LETT #1#
               (SPADCALL
                (SPADCALL (SPADCALL 2 |n| (QREFELT $ 41)) |n| (QREFELT $ 43))
...
(DEFUN |IntegerCombinatoricFunctions| (#1=#:G206)
  (PROG ()
    (RETURN
     (PROG (#2=#:G207)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (|IntegerCombinatoricFunctions;| #1#) (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
