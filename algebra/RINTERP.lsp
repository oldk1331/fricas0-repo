...
(DEFUN |RINTERP;interpolate;2L2NniF;1| (|xlist| |ylist| |m| |k| $)
  (PROG (|reslist| |i| #1=#:G136 #2=#:G135 |res| |collist| #3=#:G134 #4=#:G133
         ...
         #10=#:G127 #11=#:G126 #12=#:G125 #13=#:G124 #14=#:G123)
    (RETURN
     (SEQ
      (COND
        ...
        (SEQ
         (LETT |tempvec|
               (PROGN
                ...
                (SEQ (LETT |i| 1 . #15#) (LETT #13# (+ (+ |m| |k|) 1) . #15#)
                     G190 (COND ((|greater_SI| |i| #13#) (GO G191)))
                     (SEQ
                      (EXIT
                       (LETT #14# (CONS (|spadConstant| $ 11) #14#) . #15#)))
         ...
         (LETT |collist|
               (CONS |tempvec|
                     (PROGN
                      (LETT #12# NIL . #15#)
                      (SEQ (LETT |j| 1 . #15#) (LETT #11# (MAX |m| |k|) . #15#)
                           G190 (COND ((|greater_SI| |j| #11#) (GO G191)))
                           (SEQ
                            (EXIT
                             (LETT #12#
                                   (CONS
         ...
         (LETT |res|
               (SPADCALL
                (SPADCALL (SPADCALL |collist| (QREFELT $ 22)) (QREFELT $ 23))
                (QREFELT $ 25))
               . #15#)
         (COND
                              ...
                              (CONS
                               (SPADCALL
                                (SPADCALL (SPADCALL |res| 1 (QREFELT $ 31))
                                          (+ |i| 1) (QREFELT $ 32))
                                (SPADCALL
                                 (SPADCALL (QREFELT $ 6) (QREFELT $ 35)) |i|
                                 (QREFELT $ 36))
                                (QREFELT $ 37))
                               #2#)
                              . #15#)))
  ...
  (PROG (|pv$| $ |dv$| DV$2 DV$1)
    (RETURN
     (PROGN
      ...
      (|haddProp| |$ConstructorCache| '|RationalInterpolation| (LIST DV$1 DV$2)
                  (CONS 1 $))
      (|stuffDomainSlots| $)
      (QSETREFV $ 6 |#1|)
      (QSETREFV $ 7 |#2|)
