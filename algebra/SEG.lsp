...
(DEFUN |SEG;coerce;$Of;10| (|s| $)
  (PROG (|seg|)
    (RETURN
     (SEQ
      (LETT |seg|
            (SPADCALL (SPADCALL (QVELT |s| 0) (QREFELT $ 21))
                      (SPADCALL (QVELT |s| 1) (QREFELT $ 21)) (QREFELT $ 22))
            |SEG;coerce;$Of;10|)
      (EXIT
       (COND ((EQL (QVELT |s| 2) 1) |seg|)
             ('T
              (SPADCALL " by " |seg| (SPADCALL (QVELT |s| 2) (QREFELT $ 23))
                        (QREFELT $ 24))))))))) 

(PUT '|SEG;convert;S$;11| '|SPADreplace| '(XLAM (|a|) (VECTOR |a| |a| 1))) 

(DEFUN |SEG;convert;S$;11| (|a| $) (VECTOR |a| |a| 1)) 

(DEFUN |SEG;convert;$If;12| (|s| $)
  (PROG (|seg|)
    (RETURN
     (SEQ
      (LETT |seg|
            (SPADCALL 'SEGMENT
                      (LIST
                       (SPADCALL (SPADCALL |s| (QREFELT $ 10)) (QREFELT $ 28))
                       (SPADCALL (SPADCALL |s| (QREFELT $ 11)) (QREFELT $ 28)))
                      (QREFELT $ 32))
            |SEG;convert;$If;12|)
      (EXIT
...
(DEFUN |SEG;expand;$L;14| (|s| $) (SPADCALL (LIST |s|) (QREFELT $ 44))) 

(DEFUN |SEG;map;M$L;15| (|f| |s| $)
  (PROG (|l| |lr| |inc| |h|)
    (RETURN
     (SEQ (LETT |lr| NIL . #1=(|SEG;map;M$L;15|))
          (LETT |l| (SPADCALL |s| (QREFELT $ 9)) . #1#)
          (LETT |h| (SPADCALL |s| (QREFELT $ 11)) . #1#)
          (LETT |inc| (SPADCALL (SPADCALL |s| (QREFELT $ 14)) (QREFELT $ 35))
                . #1#)
          (COND
           ((SPADCALL |inc| (|spadConstant| $ 37) (QREFELT $ 38))
            (SEQ G190
                 (COND ((NULL (SPADCALL |l| |h| (QREFELT $ 39))) (GO G191)))
                 (SEQ (LETT |lr| (CONS (SPADCALL |l| |f|) |lr|) . #1#)
                      (EXIT
                       (LETT |l| (SPADCALL |l| |inc| (QREFELT $ 40)) . #1#)))
                 NIL (GO G190) G191 (EXIT NIL)))
           ('T
            (SEQ G190
                 (COND ((NULL (SPADCALL |l| |h| (QREFELT $ 41))) (GO G191)))
                 (SEQ (LETT |lr| (CONS (SPADCALL |l| |f|) |lr|) . #1#)
                      (EXIT
                       (LETT |l| (SPADCALL |l| |inc| (QREFELT $ 40)) . #1#)))
                 NIL (GO G190) G191 (EXIT NIL))))
      ...
      (COND
       ((|testBitVector| |pv$| 3)
        (PROGN
         ...
         (QSETREFV $ 25 (CONS (|dispatchFunction| |SEG;coerce;$Of;10|) $)))))
      (COND
       ((|testBitVector| |pv$| 1)
        (PROGN
         (QSETREFV $ 34 (CONS (|dispatchFunction| |SEG;convert;$If;12|) $)))))
      (COND
       ((|testBitVector| |pv$| 2)
        (PROGN
