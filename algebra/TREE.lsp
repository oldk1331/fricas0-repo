...
(DEFUN |TREE;count;S$Nni;6| (|n| |t| $)
  (PROG (|i| #1=#:G165 #2=#:G164 #3=#:G166 #4=#:G172 |c|)
    (RETURN
     (SEQ
      (COND ((QEQCAR |t| 1) 0)
            (#5='T
...
(DEFUN |TREE;count;M$Nni;7| (|fn| |t| $)
  (PROG (|i| #1=#:G174 #2=#:G173 #3=#:G175 #4=#:G179 |c|)
    (RETURN
     (SEQ
      (COND ((QEQCAR |t| 1) 0)
            (#5='T
...
(DEFUN |TREE;distance1| (|t1| |t2| $)
  (PROG (#1=#:G207 #2=#:G206 #3=#:G208 #4=#:G213 #5=#:G103 |u| |n| #6=#:G212
         |t| #7=#:G211)
    (RETURN
     (SEQ
...
(DEFUN |TREE;leaves;$L;19| (|t| $)
  (PROG (#1=#:G232 #2=#:G231 #3=#:G233 #4=#:G236 |c|)
    (RETURN
     (SEQ
      (COND ((QEQCAR |t| 1) NIL)
...
(DEFUN |TREE;nodes;$L;22| (|t| $)
  (PROG (#1=#:G241 #2=#:G240 #3=#:G242 #4=#:G247 #5=#:G104 |nl| #6=#:G246 |c|
         #7=#:G245)
    (RETURN
     (SEQ
...
(DEFUN |TREE;parts;$L;28| (|t| $)
  (PROG (#1=#:G280 #2=#:G279 #3=#:G281 #4=#:G286 #5=#:G105 |u| #6=#:G285 |c|
         #7=#:G284)
    (RETURN
     (SEQ
      (COND ((QEQCAR |t| 1) NIL)
...
(DEFUN |TREE;equal?| (|t1| |t2| |ot1| |ot2| |k| $)
  (PROG (#1=#:G299 #2=#:G300 |x| #3=#:G301 |y| #4=#:G297 |c2| |c1|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |TREE;treeCount| (|t| |origTree| |k| $)
  (PROG (#1=#:G311 #2=#:G310 #3=#:G312 #4=#:G315 |c|)
    (RETURN
     (SEQ
      (COND
       ((EQL |k| 5)
...
(DEFUN |TREE;coerce1| (|t| |parents| |pl| $)
  (PROG (#1=#:G338 |br| #2=#:G337 |nodeForm| |k|)
    (RETURN
     (SEQ
...
(DEFUN |TREE;cyclicEqual4?| (|t1| |t2| |cp1| |cp2| $)
  (PROG (#1=#:G392 #2=#:G393 |x| #3=#:G394 |y| |k|)
    (RETURN
     (SEQ
      (EXIT
       (COND ((QEQCAR |t1| 1) (QEQCAR |t2| 1)) ((QEQCAR |t2| 1) 'NIL)
...
(DEFUN |TREE;cyclicParents3| (|x| |parents| |pl| $)
  (PROG (#1=#:G406 |y| |cycleMembers| #2=#:G405 #3=#:G404)
    (RETURN
     (SEQ
...
(DEFUN |Tree;| (|#1|)
  (PROG (|pv$| #1=#:G427 #2=#:G428 #3=#:G429 $ |dv$| DV$1)
    (RETURN
     (PROGN
