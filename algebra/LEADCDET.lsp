...
(DEFUN |LEADCDET;polCase;ZNniLB;1| (|d| |nk| |lval| $)
  (PROG (|distlist| #1=#:G142 |q| |y| #2=#:G134 #3=#:G143 #4=#:G133 |j| |i|)
    (RETURN
     (SEQ
      (EXIT
                 ...
                 (SEQ
                  ...
                  (SEQ (LETT |j| 0 . #5#)
                       ...
                       (SEQ
                        ...
                        (EXIT
                         (COND
                          ((SPADCALL |q| (|spadConstant| $ 15) (QREFELT $ 21))
      ...
      (EXIT
       (SEQ (LETT |i| 0 . #2=(|LEADCDET;checkpow|)) G190 NIL
            (SEQ (LETT |qt| (SPADCALL |b| |a| (QREFELT $ 25)) . #2#)
                 (COND ((QEQCAR |qt| 1) (PROGN (LETT #1# |i| . #2#) (GO #1#))))
                 (EXIT (LETT |b| (QCDR |qt|) . #2#)))
            (LETT |i| (|inc_SI| |i|) . #2#) (GO G190) G191 (EXIT NIL)))
      #1# (EXIT #1#))))) 

(DEFUN |LEADCDET;distribute| (|contm| |unilist| |pl| |vl| |lvar| |lval| $)
  (PROG (|i| |d| |lcp| |nf|)
    (RETURN
          ...
          (SEQ (LETT |i| 1 . #1#) G190
               ...
               (SEQ
                ...
                (LETT |d|
                      ...
                      . #1#)
                (SPADCALL |unilist| |i|
                          (SPADCALL |d| (SPADCALL |unilist| |i| (QREFELT $ 28))
                                    (QREFELT $ 34))
                          (QREFELT $ 35))
                (EXIT
                 (LETT |contm| (SPADCALL |contm| |d| (QREFELT $ 20)) . #1#)))
...
(DEFUN |LEADCDET;distFact;ZLRLLLU;4|
  ...
  (PROG (|i| #1=#:G191 #2=#:G185 #3=#:G190 |k| |c| |d| |h| #4=#:G189 |aux|
         #5=#:G188 |vlp| #6=#:G187 |lexp| |lpol| #7=#:G186 |fpl| |nf|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ (LETT |lpol| NIL . #8=(|LEADCDET;distFact;ZLRLLLU;4|))
            ...
            (SEQ (LETT |fpl| NIL . #8#) (LETT #7# (QCDR |plead|) . #8#) G190
                 (COND
                  ((OR (ATOM #7#) (PROGN (LETT |fpl| (CAR #7#) . #8#) NIL))
                   (GO G191)))
                 (SEQ (LETT |lpol| (CONS (QCAR |fpl|) |lpol|) . #8#)
                      (EXIT (LETT |lexp| (CONS (QCDR |fpl|) |lexp|) . #8#)))
            ...
            (LETT |vlp|
                  (PROGN
                   ...
                   (SEQ (LETT |i| 1 . #8#) G190
                        ...
                        (SEQ
                         (EXIT
                          (LETT #6# (CONS (|spadConstant| $ 15) #6#) . #8#)))
            ...
            (LETT |aux|
                  (PROGN
                   ...
                   (SEQ (LETT |i| 1 . #8#) G190
                        ...
                        (SEQ
                         (EXIT
                          (LETT #5# (CONS (|spadConstant| $ 16) #5#) . #8#)))
                        (LETT |i| (|inc_SI| |i|) . #8#) (GO G190) G191
                        (EXIT (NREVERSE #5#))))
                  . #8#)
            (SEQ (LETT |i| 1 . #8#) G190
                  ...
                  (COND
                   ((OR (SPADCALL |c| (|spadConstant| $ 15) (QREFELT $ 21))
                        (SPADCALL |c|
                                  ...
                                  (QREFELT $ 21)))
                    (EXIT "next i")))
                  (SEQ (LETT |k| 1 . #8#) (LETT #4# (LENGTH |lpol|) . #8#) G190
                              ...
                              ((SPADCALL |h| 0 (QREFELT $ 42))
                               (SEQ
                                ...
                                (SPADCALL |vlp| |i|
                                          ...
                                          (QREFELT $ 48))
                                (EXIT
                                 (LETT |c| (SPADCALL |c| |d| (QREFELT $ 20))
                                       . #8#)))))))))))
                  ...
                  (EXIT
                   (COND
                    ((SPADCALL (SPADCALL |lexp| |k| (QREFELT $ 39)) 0
                               (QREFELT $ 49))
                     (PROGN (LETT #2# (CONS 1 "failed") . #8#) (GO #2#))))))
...
(DEFUN |LeadingCoefDetermination| (&REST #1=#:G192)
  (PROG ()
    (RETURN
     (PROG (#2=#:G193)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
            ...
            (COND
             ((NOT #2#)
              (HREM |$ConstructorCache| '|LeadingCoefDetermination|))))))))))) 
