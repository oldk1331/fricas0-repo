...
(DEFUN |PATRES;failed;$;2| ($) (CONS 1 "failed")) 

(PUT '|PATRES;failed?;$B;3| '|SPADreplace| '(XLAM (|x|) (QEQCAR |x| 1))) 

(DEFUN |PATRES;failed?;$B;3| (|x| $) (QEQCAR |x| 1)) 

(DEFUN |PATRES;insertMatch;PS2$;4| (|p| |x| |l| $)
  (CONS 0
        (SPADCALL (CONS (SPADCALL |p| (QREFELT $ 17)) |x|) (QCDR |l|)
                  (QREFELT $ 19)))) 

(DEFUN |PATRES;construct;L$;5| (|l| $) (CONS 0 (SPADCALL |l| (QREFELT $ 22)))) 
              ...
              (SEQ (LETT |lr| (QCDR |r|) . #4=(|PATRES;satisfy?;$PU;7|))
                   (LETT |lv|
                         (PROGN
                          (LETT #3# NIL . #4#)
                          (SEQ (LETT |v| NIL . #4#)
                               (LETT #2# (QCAR (SPADCALL |p| (QREFELT $ 27)))
                                     . #4#)
                               G190
                               (COND
                                ((OR (ATOM #2#)
...
(DEFUN |PATRES;addMatch;PS2$;12| (|p| |x| |l| $)
  (PROG (|r| |sy| |al|)
    (RETURN
     (SEQ
      (COND
...
(DEFUN |PATRES;getMatch;P$U;13| (|p| |l| $)
  ...
  (PROG (|pv$| $ |dv$| DV$2 DV$1)
    (RETURN
     (PROGN
      ...
      (|haddProp| |$ConstructorCache| '|PatternMatchResult| (LIST DV$1 DV$2)
                  (CONS 1 $))
      (|stuffDomainSlots| $)
      (QSETREFV $ 6 |#1|)
      (QSETREFV $ 7 |#2|)
...
(MAKEPROP '|PatternMatchResult| '|infovec|
          (LIST
           ...
           '#(~= 81 |union| 87 |satisfy?| 93 |new| 99 |latex| 103 |insertMatch|
              108 |hash| 115 |getMatch| 120 |failed?| 126 |failed| 131
                       ...
                       (CONS
                        ...
                        (|makeByteWordVec2| 50
                                            '(0 9 0 10 1 16 15 0 17 2 9 0 18 0
                                              ...
                                              16 7 0 20 1 0 50 0 1 2 0 28 16 0
                                              48 1 0 13 0 14 0 0 0 12 1 0 21 0
                                              25 1 0 0 21 23 1 0 40 0 42 4 0 0
                                              16 7 0 7 46 3 0 0 16 7 0 45 2 0
                                              13 0 0 39)))))
           '|lookupComplete|)) 
