...
(DEFUN |ACPLOT;findPtOnList| (|pt| |pointList| $)
  (PROG (#1=#:G163 #2=#:G165 #3=#:G166 |point|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |ACPLOT;listPtsOnHorizBdry| (|pRN| |y| |y0| |xMinNF| |xMaxNF| $)
  (PROG (|pointList| #1=#:G191 |root| |roots| |f| |ySF|)
    (RETURN
...
(DEFUN |ACPLOT;listPtsOnVertBdry| (|pRN| |x| |x0| |yMinNF| |yMaxNF| $)
  (PROG (|pointList| #1=#:G195 |root| |roots| |f| |xSF|)
    (RETURN
...
(DEFUN |ACPLOT;listPtsInRect| (|points| |xMin| |xMax| |yMin| |yMax| $)
  (PROG (|pointList| |yy| |xx| #1=#:G200 |point|)
    (RETURN
...
(DEFUN |ACPLOT;makeOneVarSketch|
  ...
  (PROG (|bran| |branch| |lf| |pt2| |rt| |pt1| #1=#:G255 |rootSF| |xMaxSF|
         |xMinSF| |bt| |tp| #2=#:G254 |yMaxSF| |yMinSF| |sketchRoots| #3=#:G253
...
(DEFUN |ACPLOT;singValBetween?| (|xCurrent| |xNext| |xSingList| $)
  (PROG (#1=#:G263 #2=#:G265 #3=#:G266 |xVal|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |ACPLOT;segmentInfo|
  ...
  (PROG (#1=#:G283 |val| |nextFrom| |nxt| |top| |bot| |currentFrom| |current|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ G190 NIL
...
(DEFUN |ACPLOT;makeRatFcnSketch|
  ...
  (PROG (|bran| |curve| |RFPlot| #1=#:G340 |segment| #2=#:G325 |topList|
         |botList| |lo| |segList| |segInfo| |maxSF| |minSF| |hi| |top| |bot|
         #3=#:G332 |tp| #4=#:G339 |val| |bt| #5=#:G338 |rt| |lf| #6=#:G337
         #7=#:G336 |rightVal| |leftVal| |denUPolyRN| |outList| #8=#:G335 |root|
         |roots| #9=#:G334 |singList| #10=#:G333 |vtans| |htans| |vtanPts|
...
(DEFUN |ACPLOT;listPtsOnSegment|
       (|pSF| |dpdxSF| |dpdySF| |x| |y| |p0| |p1| |corners| |delta| |err|
        |bound| |crits| |bdry| $)
  (PROG (|pointList| #1=#:G397 |p2| |ptInfo| |pt1| |pt0|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |ACPLOT;listPtsOnLoop|
       (|pSF| |dpdxSF| |dpdySF| |x| |y| |p1| |corners| |delta| |err| |bound|
        |crits| |bdry| $)
  (PROG (|pointList| #1=#:G410 |p2| |ptInfo| |pt1| |pt0| |p0| |py| |px| |y1|
         |x1|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |ACPLOT;computeNextPt|
       (|pSF| |dpdxSF| |dpdySF| |x| |y| |p0| |p1| |corners| |delta| |err|
        |bound| |crits| |bdry| $)
  (PROG (|critPt| |bdryPt| |pt| |x2| #1=#:G465 |x2New| |f| |y2| |y2New|
         |y2Approx| |x2Approx| |lookingFor| |incVar| #2=#:G437 |critPt2|
         |critPt1| |pt2| |pt1| |x2Temp| |y2Temp| |yHi| |yLo| |yPointList| |xHi|
         |xLo| |xPointList| |yy| |xx| #3=#:G466 |y2Approxx| |x2Approxx|
...
(DEFUN |ACPLOT;newtonApprox| (|f| |a0| |err| |bound| $)
  (PROG (|i| |newApprox| |oldApprox| #1=#:G473 |Df|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |ACPLOT;coerce;$Of;42| (|acplot| $)
  (PROG (|ff| |f| |ll| #1=#:G482 |p| #2=#:G481 #3=#:G480 |branch| |l| |les|
