
(PUT '|ROOTUT;root_pair| '|SPADreplace| 'CONS) 

(SDEFUN |ROOTUT;root_pair|
        ((|a| (F)) (|b| (F)) (% (|Record| (|:| |real| F) (|:| |imag| F))))
        (CONS |a| |b|)) 

(SDEFUN |ROOTUT;root4| ((|a| (F)) (% (F)))
        (SPROG
         ((|p1| (|SparseUnivariatePolynomial| F))
          (|rec|
           (|Record| (|:| |exponent| (|NonNegativeInteger|)) (|:| |coef| F)
                     (|:| |radicand| F))))
         (SEQ (LETT |rec| (SPADCALL |a| 4 (QREFELT % 14)))
              (LETT |p1|
                    (SPADCALL
                     (SPADCALL (|spadConstant| % 16) (QVELT |rec| 0)
                               (QREFELT % 18))
                     (SPADCALL (QVELT |rec| 2) (QREFELT % 19)) (QREFELT % 20)))
              (EXIT
               (SPADCALL (QVELT |rec| 1) (SPADCALL |p1| (QREFELT % 22))
                         (QREFELT % 23)))))) 

(SDEFUN |ROOTUT;my_sqrt| ((|a| (F)) (% (F)))
        (SPROG
         ((|fr1| #1=(|SparseUnivariatePolynomial| F))
          (|fl|
           (|List|
            (|Record| (|:| |flag| (|Union| "nil" "sqfr" "irred" "prime"))
                      (|:| |factor| #1#)
                      (|:| |exponent| (|NonNegativeInteger|)))))
          (|fr| (|Factored| (|SparseUnivariatePolynomial| F)))
          (|p| (|SparseUnivariatePolynomial| F)))
         (SEQ
          (LETT |p|
                (SPADCALL (SPADCALL (|spadConstant| % 16) 2 (QREFELT % 18))
                          (SPADCALL |a| (QREFELT % 19)) (QREFELT % 20)))
          (LETT |fr| (SPADCALL |p| (QREFELT % 25)))
          (LETT |fl| (SPADCALL |fr| (QREFELT % 30)))
          (EXIT
           (COND ((EQL (LENGTH |fl|) 1) (SPADCALL |a| (QREFELT % 31)))
                 (#2='T
                  (SEQ (LETT |fr1| (QVELT (SPADCALL |fl| 1 (QREFELT % 33)) 1))
                       (EXIT
                        (COND
                         ((SPADCALL (SPADCALL |fr1| (QREFELT % 34)) 1
                                    (QREFELT % 36))
                          (|error| "impossible"))
                         (#2#
                          (SPADCALL
                           (SPADCALL (SPADCALL |fr1| 0 (QREFELT % 38))
                                     (SPADCALL |fr1| 1 (QREFELT % 38))
                                     (QREFELT % 39))
                           (QREFELT % 40)))))))))))) 

(SDEFUN |ROOTUT;my_root3;2F;4| ((|a| (F)) (% (F)))
        (SPROG
         ((|fr1| #1=(|SparseUnivariatePolynomial| F))
          (|fl|
           (|List|
            (|Record| (|:| |flag| (|Union| "nil" "sqfr" "irred" "prime"))
                      (|:| |factor| #1#)
                      (|:| |exponent| (|NonNegativeInteger|)))))
          (|fr| (|Factored| (|SparseUnivariatePolynomial| F)))
          (|p| (|SparseUnivariatePolynomial| F)))
         (SEQ
          (LETT |p|
                (SPADCALL (SPADCALL (|spadConstant| % 16) 2 (QREFELT % 18))
                          (SPADCALL |a| (QREFELT % 19)) (QREFELT % 20)))
          (LETT |fr| (SPADCALL |p| (QREFELT % 25)))
          (LETT |fl| (SPADCALL |fr| (QREFELT % 30)))
          (EXIT
           (COND
            ((EQL (LENGTH |fl|) 1)
             (SPADCALL |a| (SPADCALL 1 3 (QREFELT % 44)) (QREFELT % 45)))
            (#2='T
             (SEQ (LETT |fr1| (QVELT (SPADCALL |fl| 1 (QREFELT % 33)) 1))
                  (COND
                   ((SPADCALL (SPADCALL |fr1| (QREFELT % 34)) 1 (QREFELT % 36))
                    (LETT |fr1| (QVELT (SPADCALL |fl| 2 (QREFELT % 33)) 1))))
                  (EXIT
                   (COND
                    ((SPADCALL (SPADCALL |fr1| (QREFELT % 34)) 1
                               (QREFELT % 36))
                     (|error| "impossible"))
                    (#2#
                     (SPADCALL
                      (SPADCALL (SPADCALL |fr1| 0 (QREFELT % 38))
                                (SPADCALL |fr1| 1 (QREFELT % 38))
                                (QREFELT % 39))
                      (QREFELT % 40)))))))))))) 

(SDEFUN |ROOTUT;quartic3|
        ((|r1| (F)) (|a0| (F)) (|a| (F)) (|b| (F))
         (% (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
        (SPROG ((|q2| (F)) (|q1| (F)) (|qq| (F)) (|rr| (F)))
               (SEQ (LETT |rr| (|ROOTUT;my_sqrt| |r1| %))
                    (LETT |qq|
                          (SPADCALL |r1|
                                    (SPADCALL
                                     (SPADCALL (|spadConstant| % 16)
                                               (SPADCALL 2 (QREFELT % 47))
                                               (QREFELT % 39))
                                     |a| (QREFELT % 23))
                                    (QREFELT % 48)))
                    (LETT |q1|
                          (SPADCALL |qq|
                                    (SPADCALL
                                     (SPADCALL |b| |rr| (QREFELT % 23))
                                     (SPADCALL 4 |r1| (QREFELT % 50))
                                     (QREFELT % 39))
                                    (QREFELT % 48)))
                    (LETT |q2|
                          (SPADCALL |qq|
                                    (SPADCALL
                                     (SPADCALL |b| |rr| (QREFELT % 23))
                                     (SPADCALL 4 |r1| (QREFELT % 50))
                                     (QREFELT % 39))
                                    (QREFELT % 51)))
                    (EXIT
                     (LIST
                      (|ROOTUT;root_pair|
                       (SPADCALL |rr|
                                 (SPADCALL |a0| (SPADCALL 4 (QREFELT % 47))
                                           (QREFELT % 39))
                                 (QREFELT % 51))
                       (|ROOTUT;my_sqrt| |q1| %) %)
                      (|ROOTUT;root_pair|
                       (SPADCALL (SPADCALL |rr| (QREFELT % 40))
                                 (SPADCALL |a0| (SPADCALL 4 (QREFELT % 47))
                                           (QREFELT % 39))
                                 (QREFELT % 51))
                       (|ROOTUT;my_sqrt| |q2| %) %)))))) 

(SDEFUN |ROOTUT;do_f2|
        ((|del| (F)) (|a2| (F)) (|b2| (F)) (|a0| (F)) (|a| (F)) (|b| (F))
         (% (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
        (SPROG ((|d2| (F)) (|su| (|Union| (|Integer|) "failed")))
               (SEQ (LETT |d2| (|ROOTUT;my_sqrt| |del| %))
                    (LETT |su| (SPADCALL |d2| (QREFELT % 54)))
                    (EXIT
                     (COND ((QEQCAR |su| 1) NIL)
                           ('T
                            (SEQ
                             (COND
                              ((< (QCDR |su|) 0)
                               (LETT |d2| (SPADCALL |d2| (QREFELT % 40)))))
                             (EXIT
                              (|ROOTUT;quartic3|
                               (SPADCALL
                                (SPADCALL (SPADCALL |b2| (QREFELT % 40)) |d2|
                                          (QREFELT % 48))
                                (SPADCALL 2 |a2| (QREFELT % 50))
                                (QREFELT % 39))
                               |a0| |a| |b| %))))))))) 

(SDEFUN |ROOTUT;quartic2;SupL;7|
        ((|p| (|SparseUnivariatePolynomial| F))
         (% (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
        (SPROG
         ((#1=#:G75 NIL) (|su| (|Union| (|Integer|) "failed")) (|t2| (F))
          (|t1| (F)) (|del| (F)) (|b2| (F)) (|a2| (F)) (|s1| (|Integer|))
          (|r1| (F)) (|f2| #2=(|SparseUnivariatePolynomial| F)) (|f1| #2#)
          (|#G22| #2#) (|#G21| #2#) (|cnt| (|Integer|)) (|cc| (F))
          (#3=#:G76 NIL) (|fac| NIL)
          (|fl|
           (|List|
            (|Record| (|:| |flag| (|Union| "nil" "sqfr" "irred" "prime"))
                      (|:| |factor| #2#)
                      (|:| |exponent| (|NonNegativeInteger|)))))
          (|fr| (|Factored| (|SparseUnivariatePolynomial| F)))
          (|r| (|SparseUnivariatePolynomial| F))
          (|xx| (|SparseUnivariatePolynomial| F)) (|c| (F)) (|b| (F)) (|a| (F))
          (|a0| (F)) (|lc| (F)))
         (SEQ
          (EXIT
           (SEQ (LETT |lc| (SPADCALL |p| (QREFELT % 55)))
                (LETT |p|
                      (SPADCALL
                       (SPADCALL (|spadConstant| % 16) |lc| (QREFELT % 39)) |p|
                       (QREFELT % 59)))
                (LETT |a0| (SPADCALL |p| 3 (QREFELT % 38)))
                (LETT |p|
                      (SPADCALL |p| (QREFELT % 10)
                                (SPADCALL
                                 (SPADCALL (|spadConstant| % 16) 1
                                           (QREFELT % 18))
                                 (SPADCALL
                                  (SPADCALL |a0| (SPADCALL 4 (QREFELT % 47))
                                            (QREFELT % 39))
                                  (QREFELT % 19))
                                 (QREFELT % 20))
                                (QREFELT % 60)))
                (LETT |a| (SPADCALL |p| 2 (QREFELT % 38)))
                (LETT |b| (SPADCALL |p| 1 (QREFELT % 38)))
                (LETT |c| (SPADCALL |p| 0 (QREFELT % 38)))
                (LETT |xx| (SPADCALL (|spadConstant| % 16) 1 (QREFELT % 18)))
                (LETT |r|
                      (SPADCALL
                       (SPADCALL 64 (SPADCALL |xx| 3 (QREFELT % 61))
                                 (QREFELT % 62))
                       (SPADCALL (SPADCALL 32 |a| (QREFELT % 50))
                                 (SPADCALL |xx| 2 (QREFELT % 61))
                                 (QREFELT % 59))
                       (QREFELT % 63)))
                (LETT |r|
                      (SPADCALL |r|
                                (SPADCALL
                                 (SPADCALL
                                  (SPADCALL (SPADCALL 16 |c| (QREFELT % 50))
                                            (QREFELT % 40))
                                  (SPADCALL 4 (SPADCALL |a| 2 (QREFELT % 64))
                                            (QREFELT % 50))
                                  (QREFELT % 48))
                                 |xx| (QREFELT % 59))
                                (QREFELT % 63)))
                (LETT |r|
                      (SPADCALL |r|
                                (SPADCALL (SPADCALL |b| 2 (QREFELT % 64))
                                          (QREFELT % 19))
                                (QREFELT % 20)))
                (LETT |fr| (SPADCALL |r| (QREFELT % 25)))
                (LETT |fl| (SPADCALL |fr| (QREFELT % 30)))
                (EXIT
                 (COND ((EQL (LENGTH |fl|) 1) NIL)
                       ((EQL (LENGTH |fl|) 3)
                        (SEQ (LETT |cnt| 0)
                             (SEQ (LETT |fac| NIL) (LETT #3# |fl|) G190
                                  (COND
                                   ((OR (ATOM #3#)
                                        (PROGN (LETT |fac| (CAR #3#)) NIL))
                                    (GO G191)))
                                  (SEQ (LETT |f1| (QVELT |fac| 1))
                                       (LETT |cc|
                                             (SPADCALL
                                              (SPADCALL |f1| 0 (QREFELT % 38))
                                              (SPADCALL |f1| 1 (QREFELT % 38))
                                              (QREFELT % 39)))
                                       (LETT |su|
                                             (SPADCALL |cc| (QREFELT % 54)))
                                       (EXIT
                                        (COND
                                         ((QEQCAR |su| 1)
                                          (PROGN (LETT #1# NIL) (GO #4=#:G74)))
                                         ((< (QCDR |su|) 0)
                                          (COND
                                           ((> |cnt| 0)
                                            (PROGN (LETT #1# NIL) (GO #4#)))
                                           ('T
                                            (SEQ (LETT |cnt| 1)
                                                 (EXIT (LETT |r1| |cc|)))))))))
                                  (LETT #3# (CDR #3#)) (GO G190) G191
                                  (EXIT NIL))
                             (EXIT
                              (COND ((EQL |cnt| 0) (|error| "impossible"))
                                    (#5='T
                                     (|ROOTUT;quartic3|
                                      (SPADCALL |r1| (QREFELT % 40)) |a0| |a|
                                      |b| %))))))
                       ((EQL (LENGTH |fl|) 2)
                        (SEQ (LETT |f1| (QVELT (|SPADfirst| |fl|) 1))
                             (LETT |f2|
                                   (QVELT (SPADCALL |fl| (QREFELT % 65)) 1))
                             (COND
                              ((EQL (SPADCALL |f2| (QREFELT % 34)) 1)
                               (PROGN
                                (LETT |#G21| |f2|)
                                (LETT |#G22| |f1|)
                                (LETT |f1| |#G21|)
                                (LETT |f2| |#G22|))))
                             (LETT |r1|
                                   (SPADCALL
                                    (SPADCALL (SPADCALL |f1| 0 (QREFELT % 38))
                                              (SPADCALL |f1| 1 (QREFELT % 38))
                                              (QREFELT % 39))
                                    (QREFELT % 40)))
                             (LETT |su| (SPADCALL |r1| (QREFELT % 54)))
                             (EXIT
                              (COND
                               ((QEQCAR |su| 1)
                                (PROGN (LETT #1# NIL) (GO #4#)))
                               (#5#
                                (SEQ (LETT |s1| (QCDR |su|))
                                     (LETT |a2|
                                           (SPADCALL |f2| 2 (QREFELT % 38)))
                                     (LETT |b2|
                                           (SPADCALL |f2| 1 (QREFELT % 38)))
                                     (LETT |del|
                                           (SPADCALL
                                            (SPADCALL |b2| 2 (QREFELT % 64))
                                            (SPADCALL
                                             (SPADCALL 4 |a2| (QREFELT % 50))
                                             (SPADCALL |f2| 0 (QREFELT % 38))
                                             (QREFELT % 23))
                                            (QREFELT % 51)))
                                     (SEQ
                                      (LETT |su|
                                            (SPADCALL |del| (QREFELT % 54)))
                                      (EXIT
                                       (COND
                                        ((NULL (QEQCAR |su| 1))
                                         (SEQ
                                          (COND
                                           ((< (QCDR |su|) 0)
                                            (PROGN
                                             (LETT #1#
                                                   (COND
                                                    ((< 0 |s1|)
                                                     (|ROOTUT;quartic3| |r1|
                                                      |a0| |a| |b| %))
                                                    (#5#
                                                     (|error| "impossible"))))
                                             (GO #4#))))
                                          (EXIT
                                           (COND
                                            ((< 0 |s1|)
                                             (SEQ
                                              (LETT |t1|
                                                    (SPADCALL |r1|
                                                              (SPADCALL |b2|
                                                                        (SPADCALL
                                                                         2 |a2|
                                                                         (QREFELT
                                                                          %
                                                                          50))
                                                                        (QREFELT
                                                                         % 39))
                                                              (QREFELT % 48)))
                                              (LETT |t2|
                                                    (SPADCALL
                                                     (SPADCALL |del|
                                                               (SPADCALL
                                                                (SPADCALL 4
                                                                          |a2|
                                                                          (QREFELT
                                                                           %
                                                                           50))
                                                                |a2|
                                                                (QREFELT % 23))
                                                               (QREFELT % 39))
                                                     (SPADCALL |t1| |t1|
                                                               (QREFELT % 23))
                                                     (QREFELT % 51)))
                                              (LETT |su|
                                                    (SPADCALL |t2|
                                                              (QREFELT % 54)))
                                              (EXIT
                                               (COND
                                                ((NULL (QEQCAR |su| 1))
                                                 (COND
                                                  ((< 0 (QCDR |su|))
                                                   (PROGN
                                                    (LETT #1#
                                                          (|ROOTUT;do_f2| |del|
                                                           |a2| |b2| |a0| |a|
                                                           |b| %))
                                                    (GO #4#)))
                                                  (#5#
                                                   (PROGN
                                                    (LETT #1#
                                                          (|ROOTUT;quartic3|
                                                           |r1| |a0| |a| |b|
                                                           %))
                                                    (GO #4#)))))))))
                                            (#5#
                                             (PROGN
                                              (LETT #1#
                                                    (|ROOTUT;do_f2| |del| |a2|
                                                     |b2| |a0| |a| |b| %))
                                              (GO #4#))))))))))
                                     (LETT |su| (SPADCALL |b2| (QREFELT % 54)))
                                     (COND
                                      ((OR (QEQCAR |su| 1)
                                           (NULL (< 0 (QCDR |su|))))
                                       (EXIT (PROGN (LETT #1# NIL) (GO #4#)))))
                                     (LETT |su|
                                           (SPADCALL
                                            (SPADCALL |a2|
                                                      (SPADCALL |f2| 0
                                                                (QREFELT % 38))
                                                      (QREFELT % 23))
                                            (QREFELT % 54)))
                                     (COND
                                      ((OR (QEQCAR |su| 1)
                                           (NULL (< 0 (QCDR |su|))))
                                       (PROGN (LETT #1# NIL) (GO #4#))))
                                     (EXIT
                                      (|ROOTUT;quartic3| |r1| |a0| |a| |b|
                                       %))))))))
                       (#5# (|error| "impossible"))))))
          #4# (EXIT #1#)))) 

(PUT '|ROOTUT;quartic2;SupL;8| '|SPADreplace| '(XLAM (|p|) NIL)) 

(SDEFUN |ROOTUT;quartic2;SupL;8|
        ((|p| (|SparseUnivariatePolynomial| F))
         (% (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
        NIL) 

(SDEFUN |ROOTUT;my_root3;2F;9| ((|a| (F)) (% (F)))
        (SPROG
         ((|rec|
           (|Record| (|:| |exponent| (|NonNegativeInteger|)) (|:| |coef| F)
                     (|:| |radicand| F))))
         (SEQ (LETT |rec| (SPADCALL |a| 3 (QREFELT % 14)))
              (EXIT
               (COND
                ((EQL (QVELT |rec| 0) 3)
                 (SPADCALL |a| (SPADCALL 1 3 (QREFELT % 44)) (QREFELT % 45)))
                ('T
                 (SPADCALL (QVELT |rec| 1) (QVELT |rec| 2) (QREFELT % 23)))))))) 

(SDEFUN |ROOTUT;qubic|
        ((|p| (|SparseUnivariatePolynomial| F))
         (%
          (|Union|
           (|Record| (|:| |reals| (|List| F))
                     (|:| |complexes|
                          (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
           "failed")))
        (SPROG
         ((|su| (|Union| (|Integer|) "failed")) (|r1| (F)) (|a| (F))
          (|rp| (|SparseUnivariatePolynomial| F)) (|a0| (F)))
         (SEQ
          (LETT |p|
                (SPADCALL
                 (SPADCALL (|spadConstant| % 16) (SPADCALL |p| (QREFELT % 55))
                           (QREFELT % 39))
                 |p| (QREFELT % 59)))
          (LETT |a0|
                (SPADCALL
                 (SPADCALL (SPADCALL |p| 2 (QREFELT % 38))
                           (SPADCALL 3 (QREFELT % 47)) (QREFELT % 39))
                 (QREFELT % 40)))
          (LETT |p|
                (SPADCALL |p| (QREFELT % 10)
                          (SPADCALL
                           (SPADCALL (|spadConstant| % 16) 1 (QREFELT % 18))
                           (SPADCALL |a0| (QREFELT % 19)) (QREFELT % 63))
                          (QREFELT % 60)))
          (EXIT
           (COND
            ((SPADCALL (LETT |rp| (SPADCALL |p| (QREFELT % 69)))
                       (QREFELT % 70))
             (SEQ (LETT |a| (SPADCALL |rp| (QREFELT % 71)))
                  (EXIT
                   (COND
                    ((SPADCALL |a| (|spadConstant| % 72) (QREFELT % 73))
                     (CONS 0 (CONS (LIST |a0| |a0| |a0|) NIL)))
                    (#1='T
                     (SEQ (LETT |su| (SPADCALL |a| (QREFELT % 54)))
                          (EXIT
                           (COND ((QEQCAR |su| 1) (CONS 1 "failed"))
                                 ((< (QCDR |su|) 0)
                                  (SEQ
                                   (LETT |r1|
                                         (SPADCALL
                                          (SPADCALL |a| (QREFELT % 40))
                                          (QREFELT % 46)))
                                   (LETT |su| (SPADCALL |r1| (QREFELT % 54)))
                                   (EXIT
                                    (COND ((QEQCAR |su| 1) (CONS 1 "failed"))
                                          (#1#
                                           (CONS 0
                                                 (CONS
                                                  (LIST
                                                   (SPADCALL |r1| |a0|
                                                             (QREFELT % 48)))
                                                  (LIST
                                                   (|ROOTUT;root_pair|
                                                    (SPADCALL
                                                     (SPADCALL
                                                      (SPADCALL |r1|
                                                                (SPADCALL 2
                                                                          (QREFELT
                                                                           %
                                                                           47))
                                                                (QREFELT % 39))
                                                      (QREFELT % 40))
                                                     |a0| (QREFELT % 48))
                                                    (SPADCALL
                                                     (SPADCALL
                                                      (SPADCALL
                                                       (SPADCALL 3
                                                                 (QREFELT %
                                                                          47))
                                                       (QREFELT % 31))
                                                      |r1| (QREFELT % 23))
                                                     (SPADCALL 2
                                                               (QREFELT % 47))
                                                     (QREFELT % 39))
                                                    %)))))))))
                                 (#1#
                                  (SEQ
                                   (LETT |r1| (SPADCALL |a| (QREFELT % 46)))
                                   (LETT |su| (SPADCALL |r1| (QREFELT % 54)))
                                   (EXIT
                                    (COND ((QEQCAR |su| 1) (CONS 1 "failed"))
                                          (#1#
                                           (CONS 0
                                                 (CONS
                                                  (LIST
                                                   (SPADCALL
                                                    (SPADCALL |r1|
                                                              (QREFELT % 40))
                                                    |a0| (QREFELT % 48)))
                                                  (LIST
                                                   (|ROOTUT;root_pair|
                                                    (SPADCALL
                                                     (SPADCALL |r1|
                                                               (SPADCALL 2
                                                                         (QREFELT
                                                                          %
                                                                          47))
                                                               (QREFELT % 39))
                                                     |a0| (QREFELT % 48))
                                                    (SPADCALL
                                                     (SPADCALL
                                                      (SPADCALL
                                                       (SPADCALL 3
                                                                 (QREFELT %
                                                                          47))
                                                       (QREFELT % 31))
                                                      |r1| (QREFELT % 23))
                                                     (SPADCALL 2
                                                               (QREFELT % 47))
                                                     (QREFELT % 39))
                                                    %)))))))))))))))))
            (#1# (CONS 1 "failed"))))))) 

(SDEFUN |ROOTUT;quartic|
        ((|p| (|SparseUnivariatePolynomial| F))
         (%
          (|Union|
           (|Record| (|:| |reals| (|List| F))
                     (|:| |complexes|
                          (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
           "failed")))
        (SPROG
         ((|res1| (|List| (|Record| (|:| |real| F) (|:| |imag| F)))) (|r1| (F))
          (|si| (|Integer|)) (|s| (|Union| (|Integer|) "failed")) (|a| (F))
          (|rp| (|SparseUnivariatePolynomial| F)))
         (SEQ
          (COND
           ((SPADCALL (LETT |rp| (SPADCALL |p| (QREFELT % 69))) (QREFELT % 70))
            (SEQ (LETT |a| (SPADCALL |rp| (QREFELT % 71)))
                 (LETT |s| (SPADCALL |a| (QREFELT % 54)))
                 (EXIT
                  (COND ((QEQCAR |s| 1) (CONS 1 "failed"))
                        (#1='T
                         (SEQ (LETT |si| (QCDR |s|))
                              (EXIT
                               (COND
                                ((EQL |si| 1)
                                 (SEQ
                                  (LETT |r1|
                                        (|ROOTUT;root4|
                                         (SPADCALL |a|
                                                   (SPADCALL
                                                    (SPADCALL 4 (QREFELT % 47))
                                                    (SPADCALL |p|
                                                              (QREFELT % 55))
                                                    (QREFELT % 23))
                                                   (QREFELT % 39))
                                         %))
                                  (EXIT
                                   (CONS 0
                                         (CONS NIL
                                               (LIST
                                                (|ROOTUT;root_pair| |r1| |r1|
                                                 %)
                                                (|ROOTUT;root_pair|
                                                 (SPADCALL |r1| (QREFELT % 40))
                                                 |r1| %)))))))
                                ((EQL |si| -1)
                                 (SEQ
                                  (LETT |r1|
                                        (SPADCALL (SPADCALL |p| (QREFELT % 22))
                                                  (QREFELT % 75)))
                                  (EXIT
                                   (CONS 0
                                         (CONS
                                          (LIST |r1|
                                                (SPADCALL |r1| (QREFELT % 40)))
                                          (LIST
                                           (|ROOTUT;root_pair|
                                            (|spadConstant| % 72) |r1| %)))))))
                                (#1# (|error| "impossible"))))))))))
           (#1#
            (SEQ (LETT |res1| (SPADCALL |p| (QREFELT % 68)))
                 (COND
                  ((NULL (NULL |res1|)) (EXIT (CONS 0 (CONS NIL |res1|)))))
                 (EXIT (CONS 1 "failed")))))))) 

(SDEFUN |ROOTUT;complex_roots;SupU;12|
        ((|p| (|SparseUnivariatePolynomial| F))
         (%
          (|Union|
           (|Record| (|:| |reals| (|List| F))
                     (|:| |complexes|
                          (|List| (|Record| (|:| |real| F) (|:| |imag| F)))))
           "failed")))
        (SPROG ((|d| (|NonNegativeInteger|)))
               (SEQ (LETT |d| (SPADCALL |p| (QREFELT % 34)))
                    (EXIT
                     (COND ((EQL |d| 3) (|ROOTUT;qubic| |p| %))
                           ((EQL |d| 4) (|ROOTUT;quartic| |p| %))
                           ('T (CONS 1 "failed"))))))) 

(DECLAIM (NOTINLINE |RootUtilities;|)) 

(DEFUN |RootUtilities| (&REST #1=#:G113)
  (SPROG NIL
         (PROG (#2=#:G114)
           (RETURN
            (COND
             ((LETT #2#
                    (|lassocShiftWithFunction| (|devaluateList| #1#)
                                               (HGET |$ConstructorCache|
                                                     '|RootUtilities|)
                                               '|domainEqualList|))
              (|CDRwithIncrement| #2#))
             ('T
              (UNWIND-PROTECT
                  (PROG1 (APPLY (|function| |RootUtilities;|) #1#)
                    (LETT #2# T))
                (COND
                 ((NOT #2#) (HREM |$ConstructorCache| '|RootUtilities|)))))))))) 

(DEFUN |RootUtilities;| (|#1| |#2|)
  (SPROG ((|pv$| NIL) (% NIL) (|dv$| NIL) (DV$2 NIL) (DV$1 NIL))
         (PROGN
          (LETT DV$1 (|devaluate| |#1|))
          (LETT DV$2 (|devaluate| |#2|))
          (LETT |dv$| (LIST '|RootUtilities| DV$1 DV$2))
          (LETT % (GETREFV 79))
          (QSETREFV % 0 |dv$|)
          (QSETREFV % 3 (LETT |pv$| (|buildPredVector| 0 0 NIL)))
          (|haddProp| |$ConstructorCache| '|RootUtilities| (LIST DV$1 DV$2)
                      (CONS 1 %))
          (|stuffDomainSlots| %)
          (QSETREFV % 6 |#1|)
          (QSETREFV % 7 |#2|)
          (SETF |pv$| (QREFELT % 3))
          (QSETREFV % 10 (SPADCALL (QREFELT % 9)))
          (COND
           ((|HasCategory| |#2| '(|PolynomialFactorizationExplicit|))
            (PROGN
             (QSETREFV % 46
                       (CONS (|dispatchFunction| |ROOTUT;my_root3;2F;4|) %))
             (QSETREFV % 68
                       (CONS (|dispatchFunction| |ROOTUT;quartic2;SupL;7|)
                             %))))
           ('T
            (PROGN
             (QSETREFV % 68
                       (CONS (|dispatchFunction| |ROOTUT;quartic2;SupL;8|) %))
             (QSETREFV % 46
                       (CONS (|dispatchFunction| |ROOTUT;my_root3;2F;9|) %)))))
          %))) 

(MAKEPROP '|RootUtilities| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL NIL (|local| |#1|) (|local| |#2|)
              (|SingletonAsOrderedSet|) (0 . |create|) '|dummy|
              (|Record| (|:| |exponent| 12) (|:| |coef| 7) (|:| |radicand| 7))
              (|NonNegativeInteger|)
              (|PolynomialRoots| (|IndexedExponents| (|Kernel| 7)) (|Kernel| 7)
                                 6 41 7)
              (4 . |froot|) (10 . |One|) (14 . |One|)
              (|SparseUnivariatePolynomial| 7) (18 . |monomial|)
              (24 . |coerce|) (29 . -) (|SparseUnivariatePolynomial| %)
              (35 . |zeroOf|) (40 . *) (|Factored| %) (46 . |factor|)
              (|Union| '"nil" '"sqfr" '"irred" '"prime")
              (|Record| (|:| |flag| 26) (|:| |factor| 17) (|:| |exponent| 12))
              (|List| 27) (|Factored| 17) (51 . |factorList|) (56 . |sqrt|)
              (|Integer|) (61 . |elt|) (67 . |degree|) (|Boolean|) (72 . ~=)
              (78 . |Zero|) (82 . |coefficient|) (88 . /) (94 . -)
              (|SparseMultivariatePolynomial| 6 (|Kernel| 7)) (99 . |One|)
              (|Fraction| 32) (103 . /) (109 . ^) (115 . |my_root3|)
              (120 . |coerce|) (125 . +) (|PositiveInteger|) (131 . *)
              (137 . -) (|Union| 32 '"failed") (|ElementaryFunctionSign| 6 7)
              (143 . |sign|) (148 . |leadingCoefficient|) (153 . |One|)
              (157 . /) (163 . |One|) (167 . *) (173 . |eval|) (180 . ^)
              (186 . *) (192 . +) (198 . ^) (204 . |second|)
              (|Record| (|:| |real| 7) (|:| |imag| 7)) (|List| 66)
              (209 . |quartic2|) (214 . |reductum|) (219 . |ground?|)
              (224 . |ground|) (229 . |Zero|) (233 . =)
              (|AlgebraicManipulations| 6 7) (239 . |rootSimp|)
              (|Record| (|:| |reals| (|List| 7)) (|:| |complexes| 67))
              (|Union| 76 '"failed") |ROOTUT;complex_roots;SupU;12|)
           '#(|quartic2| 244 |my_root3| 249 |complex_roots| 254) 'NIL
           (CONS (|makeByteWordVec2| 1 '(0))
                 (CONS '#(NIL)
                       (CONS
                        '#((|Join|
                            (|mkCategory|
                             (LIST
                              '((|complex_roots|
                                 ((|Union|
                                   (|Record| (|:| |reals| (|List| |#2|))
                                             (|:| |complexes|
                                                  (|List|
                                                   (|Record| (|:| |real| |#2|)
                                                             (|:| |imag|
                                                                  |#2|)))))
                                   "failed")
                                  (|SparseUnivariatePolynomial| |#2|)))
                                T)
                              '((|quartic2|
                                 ((|List|
                                   (|Record| (|:| |real| |#2|)
                                             (|:| |imag| |#2|)))
                                  (|SparseUnivariatePolynomial| |#2|)))
                                T)
                              '((|my_root3| (|#2| |#2|)) T))
                             (LIST) NIL NIL)))
                        (|makeByteWordVec2| 78
                                            '(0 8 0 9 2 13 11 7 12 14 0 6 0 15
                                              0 7 0 16 2 17 0 7 12 18 1 17 0 7
                                              19 2 17 0 0 0 20 1 7 0 21 22 2 7
                                              0 0 0 23 1 17 24 0 25 1 29 28 0
                                              30 1 7 0 0 31 2 28 27 0 32 33 1
                                              17 12 0 34 2 12 35 0 0 36 0 6 0
                                              37 2 17 7 0 12 38 2 7 0 0 0 39 1
                                              7 0 0 40 0 41 0 42 2 43 0 32 32
                                              44 2 7 0 0 43 45 1 0 7 7 46 1 7 0
                                              32 47 2 7 0 0 0 48 2 7 0 49 0 50
                                              2 7 0 0 0 51 1 53 52 7 54 1 17 7
                                              0 55 0 17 0 56 2 17 0 0 7 57 0 43
                                              0 58 2 17 0 7 0 59 3 17 0 0 8 0
                                              60 2 17 0 0 49 61 2 17 0 49 0 62
                                              2 17 0 0 0 63 2 7 0 0 49 64 1 28
                                              27 0 65 1 0 67 17 68 1 17 0 0 69
                                              1 17 35 0 70 1 17 7 0 71 0 7 0 72
                                              2 7 35 0 0 73 1 74 7 7 75 1 0 67
                                              17 68 1 0 7 7 46 1 0 77 17
                                              78)))))
           '|lookupComplete|)) 
