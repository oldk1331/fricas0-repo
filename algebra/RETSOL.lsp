       ...
       (SEQ (LETT |eq| NIL . #3#) (LETT #1# |l| . #3#) G190
            ...
            (SEQ
             (EXIT
              (LETT #2#
                    (CONS
                     (SPADCALL (|RETSOL;FQ2F| (SPADCALL |eq| (QREFELT $ 20)) $)
...
(DEFUN |RETSOL;solveRetract;LLL;4| (|lp| |lv| $)
  (PROG (#1=#:G119 |l| #2=#:G118 #3=#:G117 |p| #4=#:G116 |u|)
    (RETURN
     (SEQ
       ...
       (COND
         ...
         (SPADCALL
          (PROGN
           ...
           (SEQ (LETT |p| NIL . #5#) (LETT #3# |lp| . #5#) G190
                ...
                (SEQ
                 (EXIT
                  (LETT #4# (CONS (SPADCALL |p| (QREFELT $ 24)) #4#) . #5#)))
        ...
        ('T
         (PROGN
          (LETT #2# NIL . #5#)
          (SEQ (LETT |l| NIL . #5#)
               (LETT #1# (SPADCALL (QCDR |u|) |lv| (QREFELT $ 33)) . #5#) G190
               (COND
                ((OR (ATOM #1#) (PROGN (LETT |l| (CAR #1#) . #5#) NIL))
                 (GO G191)))
               (SEQ (EXIT (LETT #2# (CONS (|RETSOL;LEQQ2F| |l| $) #2#) . #5#)))
               (LETT #1# (CDR #1#) . #5#) (GO G190) G191
               (EXIT (NREVERSE #2#))))))))))) 

(DEFUN |RETSOL;QIfCan| (|l| $)
  (PROG (|ans| #1=#:G129 |u| #2=#:G130 |p|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |RETSOL;PQIfCan| (|p| $)
  (PROG (|up| |ans| #1=#:G146 |v| |s| |r| |u|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ (LETT |u| (SPADCALL |p| (QREFELT $ 37)) . #2=(|RETSOL;PQIfCan|))
            (EXIT
             (COND
              ((QEQCAR |u| 1)
               (SEQ
              ...
              (#3#
               (SEQ
                (LETT |up|
                      (SPADCALL |p| (LETT |s| (QCDR |u|) . #2#) (QREFELT $ 45))
                      . #2#)
                (LETT |ans| (|spadConstant| $ 46) . #2#)
                (SEQ G190
                     (COND
                      ((NULL
  ...
  (PROG (|pv$| $ |dv$| DV$2 DV$1)
    (RETURN
     (PROGN
      ...
      (|haddProp| |$ConstructorCache| '|RetractSolvePackage| (LIST DV$1 DV$2)
                  (CONS 1 $))
      (|stuffDomainSlots| $)
      (QSETREFV $ 6 |#1|)
      (QSETREFV $ 7 |#2|)
