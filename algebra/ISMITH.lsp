
(SDEFUN |ISMITH;smith;MMM;1|
        ((|m| (|Matrix| (|Integer|)))
         (|full| (|Mapping| (|Matrix| (|Integer|)) (|Matrix| (|Integer|))))
         (% (|Matrix| (|Integer|))))
        (SPROG
         ((|m1| (|Matrix| (|Integer|))) (|m2| (|Matrix| (|Integer|)))
          (|m3| (|Matrix| (|Integer|))) (|j| (|NonNegativeInteger|))
          (|k| (|NonNegativeInteger|)) (|nn| (|NonNegativeInteger|))
          (|count| #1=(|Integer|)) (|res| (|Matrix| (|Integer|))) (#2=#:G9 NIL)
          (|nn2| #1#) (|i| NIL) (#3=#:G10 NIL))
         (SEQ (LETT |m1| (SPADCALL |m| (QREFELT % 7)))
              (LETT |m2| (SPADCALL |m1| (QREFELT % 9)))
              (LETT |m3| (SPADCALL |m2| |full|)) (LETT |j| (ANROWS |m|))
              (LETT |k| (ANCOLS |m|)) (LETT |nn| (MIN |j| |k|))
              (LETT |count| (- |j| (ANROWS |m2|)))
              (LETT |res| (SPADCALL |j| |k| (QREFELT % 11)))
              (SEQ (LETT |i| 1) (LETT #2# |count|) G190
                   (COND ((|greater_SI| |i| #2#) (GO G191)))
                   (SEQ (EXIT (QSETAREF2O |res| |i| |i| 1 1 1)))
                   (LETT |i| (|inc_SI| |i|)) (GO G190) G191 (EXIT NIL))
              (LETT |nn2| (- |nn| |count|))
              (SEQ (LETT |i| 1) (LETT #3# |nn2|) G190
                   (COND ((|greater_SI| |i| #3#) (GO G191)))
                   (SEQ
                    (EXIT
                     (QSETAREF2O |res| (+ |i| |count|) (+ |i| |count|)
                                 (QAREF2O |m3| |i| |i| 1 1) 1 1)))
                   (LETT |i| (|inc_SI| |i|)) (GO G190) G191 (EXIT NIL))
              (EXIT |res|)))) 

(SDEFUN |ISMITH;smith;2M;2|
        ((|m| (|Matrix| (|Integer|))) (% (|Matrix| (|Integer|))))
        (SPADCALL |m| (ELT % 15) (QREFELT % 13))) 

(SDEFUN |ISMITH;completeSmith;MMR;3|
        ((|m| (|Matrix| (|Integer|)))
         (|full|
          (|Mapping|
           (|Record| (|:| |Smith| (|Matrix| (|Integer|)))
                     (|:| |leftEqMat| (|Matrix| (|Integer|)))
                     (|:| |rightEqMat| (|Matrix| (|Integer|))))
           (|Matrix| (|Integer|))))
         (%
          (|Record| (|:| |Smith| (|Matrix| (|Integer|)))
                    (|:| |leftEqMat| (|Matrix| (|Integer|)))
                    (|:| |rightEqMat| (|Matrix| (|Integer|))))))
        (SPROG
         ((|j| (|NonNegativeInteger|)) (|k| (|NonNegativeInteger|))
          (|res0|
           (|Record| (|:| |Smith| (|Matrix| (|Integer|)))
                     (|:| |leftEqMat| (|Matrix| (|Integer|)))
                     (|:| |rightEqMat| (|Matrix| (|Integer|)))))
          (|pivotsj| #1=(|Vector| (|Integer|))) (|pivotsk| #1#)
          (|m1| (|Matrix| (|Integer|))) (|m2| (|Matrix| (|Integer|)))
          (#2=#:G51 NIL) (#3=#:G52 NIL) (|lj0| (|List| (|Integer|)))
          (#4=#:G53 NIL) (#5=#:G54 NIL) (|s| (|Integer|))
          (|lj1| (|List| (|Integer|))) (#6=#:G55 NIL) (#7=#:G56 NIL)
          (|lj2| (|List| (|Integer|))) (#8=#:G57 NIL) (#9=#:G58 NIL)
          (|lk0| (|List| (|Integer|))) (|ljs| #10=(|List| (|Integer|)))
          (|lks| #10#) (|m3| (|Matrix| (|Integer|)))
          (|m4| (|Matrix| (|Integer|))) (|count| (|NonNegativeInteger|))
          (|m5| #11=(|Matrix| (|Integer|)))
          (|res1|
           (|Record| (|:| |Smith| (|Matrix| (|Integer|)))
                     (|:| |leftEqMat| (|Matrix| (|Integer|)))
                     (|:| |rightEqMat| (|Matrix| (|Integer|)))))
          (|m6| #11#) (|m7| #12=(|Matrix| (|Integer|)))
          (|m8| #13=(|Matrix| (|Integer|))) (|m9| #11#) (|m10| #12#)
          (|m11| #13#) (|m12| (|Matrix| (|Integer|))) (#14=#:G60 NIL)
          (#15=#:G59 NIL) (|m13| #12#) (|m14| #16=(|Matrix| (|Integer|)))
          (|m15| #12#) (|m16| #16#) (|i| NIL) (#17=#:G61 NIL) (#18=#:G62 NIL)
          (#19=#:G63 NIL) (#20=#:G64 NIL) (|l| NIL) (#21=#:G65 NIL)
          (|pp| (|Integer|)) (#22=#:G66 NIL) (|n| NIL) (#23=#:G67 NIL))
         (SEQ (LETT |j| (ANROWS |m|)) (LETT |k| (ANCOLS |m|))
              (EXIT
               (COND
                ((> |j| |k|)
                 (SEQ
                  (LETT |res0|
                        (SPADCALL (SPADCALL |m| (QREFELT % 17)) |full|
                                  (QREFELT % 20)))
                  (EXIT
                   (VECTOR (SPADCALL (QVELT |res0| 0) (QREFELT % 17))
                           (SPADCALL (QVELT |res0| 2) (QREFELT % 17))
                           (SPADCALL (QVELT |res0| 1) (QREFELT % 17))))))
                ('T
                 (SEQ (LETT |pivotsj| (SPADCALL |j| (QREFELT % 22)))
                      (LETT |pivotsk| (SPADCALL |k| (QREFELT % 22)))
                      (LETT |m1| (SPADCALL |m| (QREFELT % 7)))
                      (LETT |m2|
                            (SPADCALL |m1| |pivotsj| |pivotsk| (QREFELT % 23)))
                      (LETT |lj0|
                            (PROGN
                             (LETT #2# NIL)
                             (SEQ (LETT |i| 1) (LETT #3# |j|) G190
                                  (COND ((|greater_SI| |i| #3#) (GO G191)))
                                  (SEQ
                                   (EXIT
                                    (COND
                                     ((EQL
                                       (SPADCALL |pivotsj| |i| (QREFELT % 25))
                                       0)
                                      (LETT #2# (CONS |i| #2#))))))
                                  (LETT |i| (|inc_SI| |i|)) (GO G190) G191
                                  (EXIT (NREVERSE #2#)))))
                      (LETT |lj1|
                            (PROGN
                             (LETT #4# NIL)
                             (SEQ (LETT |i| 1) (LETT #5# |j|) G190
                                  (COND ((|greater_SI| |i| #5#) (GO G191)))
                                  (SEQ
                                   (EXIT
                                    (COND
                                     ((SPADCALL
                                       (LETT |s|
                                             (SPADCALL |pivotsj| |i|
                                                       (QREFELT % 25)))
                                       0 (QREFELT % 27))
                                      (LETT #4# (CONS |s| #4#))))))
                                  (LETT |i| (|inc_SI| |i|)) (GO G190) G191
                                  (EXIT (NREVERSE #4#)))))
                      (LETT |lj2|
                            (PROGN
                             (LETT #6# NIL)
                             (SEQ (LETT |i| 1) (LETT #7# |j|) G190
                                  (COND ((|greater_SI| |i| #7#) (GO G191)))
                                  (SEQ
                                   (EXIT
                                    (COND
                                     ((SPADCALL
                                       (SPADCALL |pivotsj| |i| (QREFELT % 25))
                                       0 (QREFELT % 27))
                                      (LETT #6# (CONS |i| #6#))))))
                                  (LETT |i| (|inc_SI| |i|)) (GO G190) G191
                                  (EXIT (NREVERSE #6#)))))
                      (LETT |lk0|
                            (PROGN
                             (LETT #8# NIL)
                             (SEQ (LETT |i| 1) (LETT #9# |k|) G190
                                  (COND ((|greater_SI| |i| #9#) (GO G191)))
                                  (SEQ
                                   (EXIT
                                    (COND
                                     ((EQL
                                       (SPADCALL |pivotsk| |i| (QREFELT % 25))
                                       0)
                                      (LETT #8# (CONS |i| #8#))))))
                                  (LETT |i| (|inc_SI| |i|)) (GO G190) G191
                                  (EXIT (NREVERSE #8#)))))
                      (LETT |ljs| (SPADCALL |lj2| |lj0| (QREFELT % 29)))
                      (LETT |lks| (SPADCALL |lj1| |lk0| (QREFELT % 29)))
                      (LETT |m3| (SPADCALL |m1| |ljs| |lks| (QREFELT % 30)))
                      (LETT |m4|
                            (SPADCALL |m2| |ljs|
                                      (SPADCALL 1 |j| (QREFELT % 32))
                                      (QREFELT % 33)))
                      (LETT |count| (LENGTH |lj2|))
                      (LETT |m5|
                            (SPADCALL |m3|
                                      (SPADCALL (+ |count| 1) |j|
                                                (QREFELT % 32))
                                      (SPADCALL (+ |count| 1) |k|
                                                (QREFELT % 32))
                                      (QREFELT % 34)))
                      (LETT |res1| (SPADCALL |m5| |full|))
                      (LETT |m6|
                            (SPADCALL |m4|
                                      (SPADCALL (+ |count| 1) |j|
                                                (QREFELT % 32))
                                      (SPADCALL 1 |j| (QREFELT % 32))
                                      (QREFELT % 34)))
                      (LETT |m7|
                            (SPADCALL (QVELT |res1| 1) |m6| (QREFELT % 35)))
                      (LETT |m8|
                            (SPADCALL
                             (SPADCALL |m4| (SPADCALL 1 |count| (QREFELT % 32))
                                       (SPADCALL 1 |j| (QREFELT % 32))
                                       (QREFELT % 34))
                             |m7| (QREFELT % 36)))
                      (LETT |m9|
                            (SPADCALL |m3|
                                      (SPADCALL (+ |count| 1) |j|
                                                (QREFELT % 32))
                                      (SPADCALL 1 |k| (QREFELT % 32))
                                      (QREFELT % 34)))
                      (LETT |m10|
                            (SPADCALL (QVELT |res1| 1) |m9| (QREFELT % 35)))
                      (LETT |m11|
                            (SPADCALL
                             (SPADCALL |m3| (SPADCALL 1 |count| (QREFELT % 32))
                                       (SPADCALL 1 |k| (QREFELT % 32))
                                       (QREFELT % 34))
                             |m10| (QREFELT % 36)))
                      (LETT |m12| (SPADCALL |k| |k| (QREFELT % 11)))
                      (SEQ (LETT |l| NIL) (LETT #14# |lks|) (LETT |i| 1)
                           (LETT #15# |k|) G190
                           (COND
                            ((OR (|greater_SI| |i| #15#) (ATOM #14#)
                                 (PROGN (LETT |l| (CAR #14#)) NIL))
                             (GO G191)))
                           (SEQ
                            (EXIT (SPADCALL |m12| |l| |i| 1 (QREFELT % 37))))
                           (LETT |i|
                                 (PROG1 (|inc_SI| |i|) (LETT #14# (CDR #14#))))
                           (GO G190) G191 (EXIT NIL))
                      (LETT |m13|
                            (SPADCALL
                             (SPADCALL |m12| (SPADCALL 1 |k| (QREFELT % 32))
                                       (SPADCALL (+ |count| 1) |k|
                                                 (QREFELT % 32))
                                       (QREFELT % 34))
                             (QVELT |res1| 2) (QREFELT % 35)))
                      (LETT |m14|
                            (SPADCALL
                             (SPADCALL |m12| (SPADCALL 1 |k| (QREFELT % 32))
                                       (SPADCALL 1 |count| (QREFELT % 32))
                                       (QREFELT % 34))
                             |m13| (QREFELT % 40)))
                      (LETT |m15|
                            (SPADCALL
                             (SPADCALL |m11| (SPADCALL 1 |j| (QREFELT % 32))
                                       (SPADCALL (+ |count| 1) |k|
                                                 (QREFELT % 32))
                                       (QREFELT % 34))
                             (QVELT |res1| 2) (QREFELT % 35)))
                      (LETT |m16|
                            (SPADCALL
                             (SPADCALL |m11| (SPADCALL 1 |j| (QREFELT % 32))
                                       (SPADCALL 1 |count| (QREFELT % 32))
                                       (QREFELT % 34))
                             |m15| (QREFELT % 40)))
                      (SEQ (LETT |i| |count|) G190 (COND ((< |i| 1) (GO G191)))
                           (SEQ
                            (COND
                             ((EQL (QAREF2O |m16| |i| |i| 1 1) -1)
                              (SEQ
                               (SEQ (LETT |l| 1) (LETT #17# |i|) G190
                                    (COND ((|greater_SI| |l| #17#) (GO G191)))
                                    (SEQ
                                     (EXIT
                                      (QSETAREF2O |m16| |l| |i|
                                                  (-
                                                   (QAREF2O |m16| |l| |i| 1 1))
                                                  1 1)))
                                    (LETT |l| (|inc_SI| |l|)) (GO G190) G191
                                    (EXIT NIL))
                               (EXIT
                                (SEQ (LETT |l| 1) (LETT #18# |k|) G190
                                     (COND ((|greater_SI| |l| #18#) (GO G191)))
                                     (SEQ
                                      (EXIT
                                       (QSETAREF2O |m14| |l| |i|
                                                   (-
                                                    (QAREF2O |m14| |l| |i| 1
                                                             1))
                                                   1 1)))
                                     (LETT |l| (|inc_SI| |l|)) (GO G190) G191
                                     (EXIT NIL))))))
                            (EXIT
                             (COND
                              ((SPADCALL (QAREF2O |m16| |i| |i| 1 1) 1
                                         (QREFELT % 27))
                               (|error| "completeSmith: wrong diagonal"))
                              ('T
                               (SEQ
                                (SEQ (LETT |l| 1) (LETT #19# (- |i| 1)) G190
                                     (COND ((|greater_SI| |l| #19#) (GO G191)))
                                     (SEQ
                                      (LETT |pp| (QAREF2O |m16| |l| |i| 1 1))
                                      (EXIT
                                       (SEQ (LETT |n| (+ |i| 1))
                                            (LETT #20# |k|) G190
                                            (COND ((> |n| #20#) (GO G191)))
                                            (SEQ
                                             (EXIT
                                              (QSETAREF2O |m16| |l| |n|
                                                          (-
                                                           (QAREF2O |m16| |l|
                                                                    |n| 1 1)
                                                           (* |pp|
                                                              (QAREF2O |m16|
                                                                       |i| |n|
                                                                       1 1)))
                                                          1 1)))
                                            (LETT |n| (+ |n| 1)) (GO G190) G191
                                            (EXIT NIL))))
                                     (LETT |l| (|inc_SI| |l|)) (GO G190) G191
                                     (EXIT NIL))
                                (SEQ (LETT |l| 1) (LETT #21# |k|) G190
                                     (COND ((|greater_SI| |l| #21#) (GO G191)))
                                     (SEQ
                                      (LETT |pp| (QAREF2O |m14| |l| |i| 1 1))
                                      (EXIT
                                       (SEQ (LETT |n| (+ |i| 1))
                                            (LETT #22# |k|) G190
                                            (COND ((> |n| #22#) (GO G191)))
                                            (SEQ
                                             (EXIT
                                              (QSETAREF2O |m14| |l| |n|
                                                          (-
                                                           (QAREF2O |m14| |l|
                                                                    |n| 1 1)
                                                           (* |pp|
                                                              (QAREF2O |m16|
                                                                       |i| |n|
                                                                       1 1)))
                                                          1 1)))
                                            (LETT |n| (+ |n| 1)) (GO G190) G191
                                            (EXIT NIL))))
                                     (LETT |l| (|inc_SI| |l|)) (GO G190) G191
                                     (EXIT NIL))
                                (EXIT
                                 (SEQ (LETT |n| (+ |i| 1)) (LETT #23# |k|) G190
                                      (COND ((> |n| #23#) (GO G191)))
                                      (SEQ
                                       (EXIT (QSETAREF2O |m16| |i| |n| 0 1 1)))
                                      (LETT |n| (+ |n| 1)) (GO G190) G191
                                      (EXIT NIL))))))))
                           (LETT |i| (+ |i| -1)) (GO G190) G191 (EXIT NIL))
                      (EXIT (VECTOR |m16| |m8| |m14|))))))))) 

(DECLAIM (NOTINLINE |IntegerSmithNormalForm;|)) 

(DEFUN |IntegerSmithNormalForm;| ()
  (SPROG ((|dv$| NIL) (% NIL) (|pv$| NIL))
         (PROGN
          (LETT |dv$| '(|IntegerSmithNormalForm|))
          (LETT % (GETREFV 41))
          (QSETREFV % 0 |dv$|)
          (QSETREFV % 3 (LETT |pv$| (|buildPredVector| 0 0 NIL)))
          (|haddProp| |$ConstructorCache| '|IntegerSmithNormalForm| NIL
                      (CONS 1 %))
          (|stuffDomainSlots| %)
          (SETF |pv$| (QREFELT % 3))
          %))) 

(DEFUN |IntegerSmithNormalForm| ()
  (SPROG NIL
         (PROG (#1=#:G69)
           (RETURN
            (COND
             ((LETT #1# (HGET |$ConstructorCache| '|IntegerSmithNormalForm|))
              (|CDRwithIncrement| (CDAR #1#)))
             ('T
              (UNWIND-PROTECT
                  (PROG1
                      (CDDAR
                       (HPUT |$ConstructorCache| '|IntegerSmithNormalForm|
                             (LIST
                              (CONS NIL
                                    (CONS 1 (|IntegerSmithNormalForm;|))))))
                    (LETT #1# T))
                (COND
                 ((NOT #1#)
                  (HREM |$ConstructorCache| '|IntegerSmithNormalForm|)))))))))) 

(MAKEPROP '|IntegerSmithNormalForm| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL NIL (|Matrix| 24) (0 . |copy|)
              (|UnitGaussianElimination|) (5 . |pre_smith|)
              (|NonNegativeInteger|) (10 . |zero|) (|Mapping| 6 6)
              |ISMITH;smith;MMM;1| (|SmithNormalForm| 24 21 21 6)
              (16 . |smith|) |ISMITH;smith;2M;2| (21 . |transpose|)
              (|Record| (|:| |Smith| 6) (|:| |leftEqMat| 6)
                        (|:| |rightEqMat| 6))
              (|Mapping| 18 6) |ISMITH;completeSmith;MMR;3| (|Vector| 24)
              (26 . |zero|) (31 . |pre_lr|) (|Integer|) (38 . |elt|)
              (|Boolean|) (44 . ~=) (|List| 24) (50 . |concat|) (56 . |elt|)
              (|Segment| 24) (63 . SEGMENT) (69 . |elt|) (76 . |elt|) (83 . *)
              (89 . |vertConcat|) (95 . |setelt!|) (|UniversalSegment| 24)
              (103 . SEGMENT) (109 . |horizConcat|))
           '#(|smith| 115 |completeSmith| 126) 'NIL
           (CONS (|makeByteWordVec2| 1 '(0))
                 (CONS '#(NIL)
                       (CONS
                        '#((|Join|
                            (|mkCategory|
                             (LIST
                              '((|smith|
                                 ((|Matrix| (|Integer|))
                                  (|Matrix| (|Integer|))))
                                T)
                              '((|smith|
                                 ((|Matrix| (|Integer|)) (|Matrix| (|Integer|))
                                  (|Mapping| (|Matrix| (|Integer|))
                                             (|Matrix| (|Integer|)))))
                                T)
                              '((|completeSmith|
                                 ((|Record|
                                   (|:| |Smith| (|Matrix| (|Integer|)))
                                   (|:| |leftEqMat| (|Matrix| (|Integer|)))
                                   (|:| |rightEqMat| (|Matrix| (|Integer|))))
                                  (|Matrix| (|Integer|))
                                  (|Mapping|
                                   (|Record|
                                    (|:| |Smith| (|Matrix| (|Integer|)))
                                    (|:| |leftEqMat| (|Matrix| (|Integer|)))
                                    (|:| |rightEqMat| (|Matrix| (|Integer|))))
                                   (|Matrix| (|Integer|)))))
                                T))
                             (LIST) NIL NIL)))
                        (|makeByteWordVec2| 40
                                            '(1 6 0 0 7 1 8 6 6 9 2 6 0 10 10
                                              11 1 14 6 6 15 1 6 0 0 17 1 21 0
                                              10 22 3 8 6 6 21 21 23 2 21 24 0
                                              24 25 2 24 26 0 0 27 2 28 0 0 0
                                              29 3 6 0 0 28 28 30 2 31 0 24 24
                                              32 3 6 0 0 28 31 33 3 6 0 0 31 31
                                              34 2 6 0 0 0 35 2 6 0 0 0 36 4 6
                                              24 0 24 24 24 37 2 38 0 24 24 39
                                              2 6 0 0 0 40 2 0 6 6 12 13 1 0 6
                                              6 16 2 0 18 6 19 20)))))
           '|lookupComplete|)) 
