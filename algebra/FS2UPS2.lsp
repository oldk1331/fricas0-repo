...
(DEFUN |FS2UPS2;carefulNthRootIfCan| (|ups| |n| |opt_rec| |rightOnly?| $)
  (PROG (|ans| #1=#:G323 |signum| |coef| |deg| |posCheck?|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |FS2UPS2;polyToUPS| (|poly| |opt_rec| $)
  (PROG (|ans| #1=#:G426 #2=#:G413 |coef| |deg|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |FS2UPS2;powerToUPS| (|fcn| |n| |opt_rec| $)
  (PROG (|coef| |deg| |ups| #1=#:G444 #2=#:G450 |b|)
    (RETURN
     (SEQ
...
(DEFUN |FS2UPS2;handle_args| (|args| |opt_rec| $)
  (PROG (|lsers| |lsere| |lcoef| |losers| |coef| #1=#:G472 |ups| #2=#:G459
         |nsu| #3=#:G473 |arg|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |FS2UPS2;do_taylor_via_deriv| (|nf| |lsyms| |lser| $)
  (PROG (|ups| |lders| #1=#:G487 |sym| #2=#:G486)
    (RETURN
     (SEQ
      (LETT |lders|
            (PROGN
...
(DEFUN |FS2UPS2;do_taylor_via_deriv2| (|nk| |lsyms| |lser| $)
  (PROG (|ups| |lders| #1=#:G501 |sym| #2=#:G500)
    (RETURN
     (SEQ
      (LETT |lders|
            (PROGN
...
(DEFUN |FS2UPS2;convert_args| (|lsers| |lsere| |lser0| $)
  (PROG (|nargs| |lser| |lsyms| |nsym| #1=#:G507 |s| #2=#:G508 |e| #3=#:G509
         |c|)
    (RETURN
...
(DEFUN |FS2UPS2;do_diff_eq| (|ker| |losers| |lsers| |lsere| |lser0| |getEq| $)
  (PROG (|ups| |lc| #1=#:G545 |c| #2=#:G544 |nker| #3=#:G543 |lsyms| |sym|
         |ecl| #4=#:G542 #5=#:G541 |cn1| |cn1u| |cn| |z0| |nargs| |cargs|)
    (RETURN
     (SEQ
...
(DEFUN |FS2UPS2;do_weierstrass|
  ...
  (PROG (#1=#:G556 |nres2| |lsyms| |nres| |nargs| |cargs| |z_inv| |coef| |deg|
         |z_ser|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |FS2UPS2;spec_to_UPS| (|ker| |args| |opt_rec| $)
  (PROG (|nker| |nargs| |cargs| #1=#:G679 |nexpr| |narg1| |arg1| |ii| |ir| |nm|
...
(DEFUN |FS2UPS2;prim_to_UPS| (|ker| |args| |opt_rec| $)
  (PROG (|coef| #1=#:G708 |ups| #2=#:G459 |nsu| |arg| |nm|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |FS2UPS2;logToUPS| (|arg| |opt_rec| $)
  (PROG (|logTerm| |lt| #1=#:G749 #2=#:G754 #3=#:G752 |signum| |coef| |deg|
         |ups| #4=#:G459 |result|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |FS2UPS2;atancotToUPS| (|arg| |opt_rec| |plusMinus| $)
  (PROG (|res1| #1=#:G787 #2=#:G789 |ord| |coef| |ups| #3=#:G459 |result|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |FS2UPS2;powerToGenUPS| (|fcn| |n| |opt_rec| $)
  (PROG (|xOpList| |coef| |deg| |ups| #1=#:G459 #2=#:G962 |b|)
    (RETURN
     (SEQ
...
(DEFUN |FS2UPS2;logToGenUPS| (|arg| |opt_rec| $)
  (PROG (|logTerm| |term1| |pow| |mon| |negRat?| |rat| |cen| |lt| #1=#:G1006
         #2=#:G1008 |signum| |coef| |deg| |ups| #3=#:G459 |result|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ
...
(DEFUN |FS2UPS2;atancotToGenUPS| (|fe| |arg| |opt_rec| |plusMinus| $)
  (PROG (#1=#:G1022 |ord| |coef| |ups| #2=#:G459 |trouble| |result|)
    (RETURN
     (SEQ
      (LETT |result| (|FS2UPS2;iExprToGenUPS| |arg| |opt_rec| $)
...
(DEFUN |FS2UPS2;genUPSApplyIfCan| (|fcn| |arg| |fcnName| |opt_rec| $)
  (PROG (#1=#:G1027 |xOpList| |lc| |deg| |ups| #2=#:G459 |series|)
    (RETURN
     (SEQ
