...
(DEFUN |REP1;invContent| (|alpha| $)
  (PROG (|i| |k| |j| #1=#:G145 |og| |f| |n| #2=#:G137 #3=#:G135 #4=#:G134
         #5=#:G136 #6=#:G144 #7=#:G125)
    (RETURN
     (SEQ
      (LETT |n|
            (PROG1
...
(DEFUN |REP1;antisymmetricTensors;MPiM;3| (|a| |k| $)
  (PROG (|tt| |rr| |t| |r| |lt| |lr| |c| |j| |i| |b| #1=#:G150 #2=#:G149 |il|
         #3=#:G161 #4=#:G160 |m| |n|)
    (RETURN
     (SEQ (LETT |n| (QVSIZE |a|) . #5=(|REP1;antisymmetricTensors;MPiM;3|))
          (EXIT
           (COND ((EQL |k| 1) |a|)
...
(DEFUN |REP1;symmetricTensors;MPiM;5| (|a| |n| $)
  (PROG (|colemanMatrix| |help| #1=#:G168 #2=#:G167 |k| |gamma| |g| |beta| |j|
         |f| |alpha| |i| |nullMatrix| |c| |dim| #3=#:G166 |m|)
    (RETURN
     (SEQ (LETT |m| (QVSIZE |a|) . #4=(|REP1;symmetricTensors;MPiM;5|))
          (EXIT
           (COND
            ((SPADCALL |m| (SPADCALL |a| (QREFELT $ 37)) (QREFELT $ 38))
            ...
            ('T
             (SEQ
              (LETT |dim|
                    ...
                    . #4#)
              (LETT |c|
                    (SPADCALL |dim| |dim| (|spadConstant| $ 17) (QREFELT $ 39))
                    . #4#)
              (LETT |f| (MAKEARR1 |n| 0) . #4#)
              (LETT |g| (MAKEARR1 |n| 0) . #4#)
              (LETT |nullMatrix| (SPADCALL 1 1 0 (QREFELT $ 40)) . #4#)
              (SEQ (LETT |i| 1 . #4#) G190
                   (COND ((|greater_SI| |i| |dim|) (GO G191)))
                   (SEQ
                    (LETT |alpha| (SPADCALL |n| |m| (- |i| 1) (QREFELT $ 41))
                          . #4#)
     ...
     (SEQ
      (PROGN
       ...
       (SEQ (LETT |ma| NIL . #3#) (LETT #1# |la| . #3#) G190
            ...
            (SEQ
             (EXIT
              (LETT #2# (CONS (SPADCALL |ma| |k| (QREFELT $ 49)) #2#) . #3#)))
            (LETT #1# (CDR #1#) . #3#) (GO G190) G191
            (EXIT (NREVERSE #2#)))))))) 

(DEFUN |REP1;tensorProduct;3M;7| (|a| |b| $) (SPADCALL |a| |b| (QREFELT $ 51))) 
    ...
    (RETURN
     (SEQ
      (PROGN
       ...
       (SEQ (LETT |i| 1 . #3#) (LETT #1# (SPADCALL |la| (QREFELT $ 53)) . #3#)
            G190 (COND ((|greater_SI| |i| #1#) (GO G191)))
            (SEQ
             (EXIT
              (LETT #2#
                    (CONS
                     (SPADCALL (SPADCALL |la| |i| (QREFELT $ 54))
                               (SPADCALL |lb| |i| (QREFELT $ 54))
                               (QREFELT $ 52))
                     #2#)
                    . #3#)))
...
(DEFUN |REP1;tensorProduct;2M;9| (|a| $) (SPADCALL |a| |a| (QREFELT $ 52))) 

(DEFUN |REP1;tensorProduct;2L;10| (|la| $) (SPADCALL |la| |la| (QREFELT $ 55))) 

(DEFUN |REP1;permutationRepresentation;PIM;11| (|p| |n| $)
  (PROG (|i| |a| #1=#:G193 #2=#:G192)
    (RETURN
     (SEQ
      (LETT |a|
            (SPADCALL
      ...
      (SEQ (LETT |i| 1 . #3#) G190 (COND ((|greater_SI| |i| |n|) (GO G191)))
           (SEQ
            (EXIT
             (SPADCALL |a| (SPADCALL |p| |i| (QREFELT $ 60)) |i| 1
                       (QREFELT $ 61))))
           (LETT |i| (|inc_SI| |i|) . #3#) (GO G190) G191 (EXIT NIL))
      (EXIT |a|))))) 

(DEFUN |REP1;permutationRepresentation;LM;12| (|p| $)
  (PROG (|i| |a| #1=#:G200 #2=#:G199 |n|)
    (RETURN
     ...
     (SEQ
      (PROGN
       ...
       (SEQ (LETT |perm| NIL . #3#) (LETT #1# |listperm| . #3#) G190
            ...
            (SEQ
             (EXIT
              (LETT #2# (CONS (SPADCALL |perm| |n| (QREFELT $ 62)) #2#)
                    . #3#)))
            (LETT #1# (CDR #1#) . #3#) (GO G190) G191
            (EXIT (NREVERSE #2#)))))))) 

(DEFUN |REP1;permutationRepresentation;LL;14| (|listperm| $)
  (PROG (#1=#:G211 |perm| #2=#:G210)
    (RETURN
     (SEQ
      (PROGN
       ...
       (SEQ (LETT |perm| NIL . #3#) (LETT #1# |listperm| . #3#) G190
            ...
            (SEQ
             (EXIT
              (LETT #2# (CONS (SPADCALL |perm| (QREFELT $ 63)) #2#) . #3#)))
            (LETT #1# (CDR #1#) . #3#) (GO G190) G191
            (EXIT (NREVERSE #2#)))))))) 
...
(DEFUN |REP1;createGenericMatrix;NniM;15| (|m| $)
  (PROG (|sy| |le| |jof| |iof| |j| |i| |res|)
    (RETURN
     (SEQ
      (LETT |res| (SPADCALL |m| |m| (|spadConstant| $ 69) (QREFELT $ 71))
      ...
      (SEQ (LETT |i| 1 . #1#) G190 (COND ((|greater_SI| |i| |m|) (GO G191)))
           (SEQ
            (EXIT
             (SEQ (LETT |j| 1 . #1#) G190
                  (COND ((|greater_SI| |j| |m|) (GO G191)))
                  (SEQ (LETT |iof| (SPADCALL |i| (QREFELT $ 73)) . #1#)
                       (LETT |jof| (SPADCALL |j| (QREFELT $ 73)) . #1#)
                       (LETT |le| (CONS |iof| (SPADCALL |jof| (QREFELT $ 75)))
                             . #1#)
                       (LETT |sy| (SPADCALL '|x| |le| (QREFELT $ 77)) . #1#)
                       (EXIT
                        (SPADCALL |res| |i| |j| (SPADCALL |sy| (QREFELT $ 78))
                                  (QREFELT $ 79))))
                  (LETT |j| (|inc_SI| |j|) . #1#) (GO G190) G191 (EXIT NIL))))
           (LETT |i| (|inc_SI| |i|) . #1#) (GO G190) G191 (EXIT NIL))
      (EXIT |res|))))) 

(DEFUN |RepresentationPackage1| (#1=#:G217)
  (PROG ()
    (RETURN
     (PROG (#2=#:G218)
       (RETURN
...
(MAKEPROP '|RepresentationPackage1| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL NIL (|local| |#1|) (|Integer|) (|List| 7)
              ...
              (100 . |antisymmetricTensors|) (106 . |ncols|) (111 . ~=)
              (117 . |new|) (124 . |new|) (131 . |unrankImproperPartitions1|)
              (138 . |nextColeman|) (145 . ~=) (151 . |inverseColeman|)
              (158 . |coerce|) (163 . |elt|) (169 . *) (175 . +)
              |REP1;symmetricTensors;MPiM;5| |REP1;symmetricTensors;LPiL;6|
              (181 . |kroneckerProduct|) |REP1;tensorProduct;3M;7|
              (187 . |maxIndex|) (192 . |elt|) |REP1;tensorProduct;3L;8|
              |REP1;tensorProduct;2M;9| |REP1;tensorProduct;2L;10|
              (198 . |zero|) (|Permutation| 7) (204 . |eval|) (210 . |setelt|)
              |REP1;permutationRepresentation;PIM;11|
              |REP1;permutationRepresentation;LM;12| (|List| 12) (|List| 59)
              |REP1;permutationRepresentation;LIL;13|
              |REP1;permutationRepresentation;LL;14| (|Polynomial| 6)
              (218 . |Zero|) (|Matrix| 68) (222 . |new|) (|OutputForm|)
              (229 . |coerce|) (|List| 72) (234 . |list|) (|Symbol|)
              (239 . |subscript|) (245 . |coerce|) (250 . |setelt|)
              |REP1;createGenericMatrix;NniM;15|)
           '#(|tensorProduct| 258 |symmetricTensors| 280
              |permutationRepresentation| 292 |createGenericMatrix| 314
              |antisymmetricTensors| 319)
           'NIL
                       ...
                       (CONS '#()
                             (|makeByteWordVec2| 80
                                                 '(1 8 7 0 9 2 8 7 0 7 10 2 12
                                                   ...
                                                   1 26 21 0 37 2 21 20 0 0 38
                                                   3 26 0 21 21 6 39 3 12 0 21
                                                   21 7 40 3 24 8 7 7 7 41 3 24
                                                   12 8 8 12 42 2 12 20 0 0 43
                                                   3 24 8 8 8 12 44 1 6 0 7 45
                                                   2 11 7 0 7 46 2 6 0 0 0 47 2
                                                   6 0 0 0 48 2 26 0 0 0 51 1
                                                   35 7 0 53 2 35 26 0 7 54 2
                                                   12 0 21 21 58 2 59 7 0 7 60
                                                   4 12 7 0 7 7 7 61 0 68 0 69
                                                   3 70 0 21 21 68 71 1 7 72 0
                                                   73 1 74 0 72 75 2 76 0 0 74
                                                   77 1 68 0 76 78 4 70 68 0 7
                                                   7 68 79 2 0 26 26 26 52 2 0
                                                   35 35 35 55 1 0 35 35 57 1 0
                                                   26 26 56 2 0 26 26 33 49 2 0
                                                   35 35 33 50 1 0 12 8 63 2 0
                                                   12 59 7 62 1 0 64 28 67 2 0
                                                   64 65 7 66 1 0 70 21 80 2 1
                                                   26 26 33 34 2 1 35 35 33
                                                   36)))))
           '|lookupComplete|)) 
