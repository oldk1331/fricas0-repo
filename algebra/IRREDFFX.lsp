...
(DEFUN |IRREDFFX;qAdicExpansion| (|z| $)
  (PROG (|r| |qr|)
    (RETURN
     (SEQ
      (LETT |qr| (DIVIDE2 |z| (QREFELT $ 11)) . #1=(|IRREDFFX;qAdicExpansion|))
      (EXIT
       (COND
        ((ZEROP (QCDR |qr|))
         (SPADCALL (SPADCALL (|spadConstant| $ 12) 1 (QREFELT $ 14))
                   (|IRREDFFX;qAdicExpansion| (QCAR |qr|) $) (QREFELT $ 15)))
        (#2='T
         (SEQ
          (LETT |r|
                (SPADCALL (SPADCALL (QCDR |qr|) (QREFELT $ 17)) (QREFELT $ 18))
                . #1#)
          (EXIT
           (COND ((ZEROP (QCAR |qr|)) |r|)
                 (#2#
                  (SPADCALL |r|
                            (SPADCALL
                             (SPADCALL (|spadConstant| $ 12) 1 (QREFELT $ 14))
                             (|IRREDFFX;qAdicExpansion| (QCAR |qr|) $)
                             (QREFELT $ 15))
                            (QREFELT $ 19))))))))))))) 

(DEFUN |IRREDFFX;getIrredPoly| (|start| |n| $)
  (PROG (|found| |pol| |end| |mon|)
    (RETURN
     (SEQ
            ...
            (COND
             ((EQL (GCD |start| (QREFELT $ 9)) 1)
              (COND
               ((SPADCALL
                 (LETT |pol|
                       (SPADCALL |mon| (|IRREDFFX;qAdicExpansion| |start| $)
                                 (QREFELT $ 19))
                       . #1#)
                 (QREFELT $ 24))
                (LETT |found| 'T . #1#)))))
      ...
      (EXIT
       (COND
        ((SPADCALL |pol| (QREFELT $ 25)) (|error| "no irreducible poly found"))
        ('T |pol|))))))) 

(DEFUN |IRREDFFX;generateIrredPoly;PiSup;3| (|n| $)
