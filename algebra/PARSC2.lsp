...
(DEFUN |PARSC2;map;MPscPsc;1| (|f| |c| $)
...
(DEFUN |ParametricSpaceCurveFunctions2| (&REST #1=#:G129)
  (PROG ()
    (RETURN
     (PROG (#2=#:G130)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |ParametricSpaceCurveFunctions2;|) #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
...
(DEFUN |ParametricSpaceCurveFunctions2;| (|#1| |#2|)
  (PROG (|pv$| $ |dv$| DV$2 DV$1)
    (RETURN
     (PROGN
      (LETT DV$1 (|devaluate| |#1|) . #1=(|ParametricSpaceCurveFunctions2|))
