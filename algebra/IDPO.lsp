...
(PUT '|IDPO;Zero;$;19| '|SPADreplace| '(XLAM NIL NIL)) 

(DEFUN |IDPO;Zero;$;19| ($) NIL) 

(PUT '|IDPO;zero?;$B;20| '|SPADreplace| 'NULL) 

(DEFUN |IDPO;zero?;$B;20| (|x| $) (NULL |x|)) 
...
(DEFUN |IDPO;+;3$;23| (|x| |y| $)
  (PROG (#1=#:G254 |msi| |degy| |degx|)
    (RETURN
     (SEQ
      (EXIT
       (COND ((NULL |x|) |y|) ((NULL |y|) |x|)
             ('T
              (SEQ (LETT |degx| (QCAR (|SPADfirst| |x|)) . #2=(|IDPO;+;3$;23|))
...
(DEFUN |IDPO;+;3$;24| (|x| |y| $) (|IDPO;add_gen| |x| |y| $)) 

(DEFUN |IDPO;*;Pi2$;25| (|n| |x| $)
  (PROG (|a| #1=#:G259 |u| #2=#:G258)
    (RETURN
     (SEQ
      (COND ((EQL |n| 0) (|spadConstant| $ 20)) ((EQL |n| 1) |x|)
            ('T
             (PROGN
              (LETT #2# NIL . #3=(|IDPO;*;Pi2$;25|))
              (SEQ (LETT |u| NIL . #3#) (LETT #1# |x| . #3#) G190
                   ...
                   (SEQ
                    (EXIT
                     (COND
                      ((SPADCALL
                        (LETT |a| (SPADCALL |n| (QCDR |u|) (QREFELT $ 47))
                              . #3#)
...
(DEFUN |IDPO;-;2$;26| (|x| $)
  (PROG (#1=#:G263 |u| #2=#:G262)
    (RETURN
     (SEQ
      (PROGN
       (LETT #2# NIL . #3=(|IDPO;-;2$;26|))
       (SEQ (LETT |u| NIL . #3#) (LETT #1# |x| . #3#) G190
            (COND
             ((OR (ATOM #1#) (PROGN (LETT |u| (CAR #1#) . #3#) NIL))
              (GO G191)))
            (SEQ
             (EXIT
              (LETT #2#
                    (CONS
                     (CONS (QCAR |u|) (SPADCALL (QCDR |u|) (QREFELT $ 49)))
                     #2#)
                    . #3#)))
            (LETT #1# (CDR #1#) . #3#) (GO G190) G191
            (EXIT (NREVERSE #2#)))))))) 

(DEFUN |IDPO;*;I2$;27| (|n| |x| $)
  (PROG (|a| #1=#:G267 |u| #2=#:G266)
    (RETURN
     (SEQ
      (COND ((EQL |n| 0) (|spadConstant| $ 20)) ((EQL |n| 1) |x|)
            ('T
             (PROGN
              (LETT #2# NIL . #3=(|IDPO;*;I2$;27|))
              (SEQ (LETT |u| NIL . #3#) (LETT #1# |x| . #3#) G190
                   (COND
                    ((OR (ATOM #1#) (PROGN (LETT |u| (CAR #1#) . #3#) NIL))
                     (GO G191)))
...
(DEFUN |IDPO;-;3$;28| (|x| |y| $)
  (PROG (|res| |end| |endcell| |newcell| |r|)
    (RETURN
     (SEQ
      (COND ((NULL |x|) (SPADCALL |y| (QREFELT $ 50))) ((NULL |y|) |x|)
            (#1='T
             (SEQ (LETT |endcell| NIL . #2=(|IDPO;-;3$;28|))
                  (LETT |res| NIL . #2#)
                  (SEQ G190
                       (COND
                        ((NULL
                  ...
                  (COND
                   ((NULL |x|)
                    (LETT |end| (SPADCALL |y| (QREFELT $ 50)) . #2#))
                   (#1# (LETT |end| |x| . #2#)))
                  (COND ((NULL |res|) (LETT |res| |end| . #2#))
                        (#1# (RPLACD |endcell| |end|)))
                  (EXIT |res|)))))))) 

(DEFUN |IDPO;subtractIfCan;2$U;29| (|x| |y| $)
  (PROG (|res| |endcell| |newcell| |r| #1=#:G316 |ru|)
    (RETURN
     (SEQ
      (EXIT
       (COND ((NULL |y|) (CONS 0 |x|))
             (#2='T
              (SEQ (LETT |endcell| NIL . #3=(|IDPO;subtractIfCan;2$U;29|))
                   (LETT |res| NIL . #3#)
                   (SEQ G190
                        (COND
                         ((NULL
...
(DEFUN |IDPO;<;2$B;30| (|x| |y| $) (SPADCALL |x| |y| (QREFELT $ 31))) 

(DEFUN |IDPO;sup;3$;31| (|x| |y| $)
  (PROG (|u|)
    (RETURN
     (SEQ
      (COND ((NULL |y|) |x|) ((NULL |x|) |y|)
            ((SPADCALL (QCAR (|SPADfirst| |x|)) (QCAR (|SPADfirst| |y|))
                       (QREFELT $ 30))
             (CONS (|SPADfirst| |y|) (SPADCALL |x| (CDR |y|) (QREFELT $ 60))))
            ((SPADCALL (QCAR (|SPADfirst| |x|)) (QCAR (|SPADfirst| |y|))
                       (QREFELT $ 33))
             (CONS (|SPADfirst| |x|) (SPADCALL (CDR |x|) |y| (QREFELT $ 60))))
            ('T
             (SEQ
              (LETT |u|
                    (SPADCALL (QCDR (|SPADfirst| |x|)) (QCDR (|SPADfirst| |y|))
              ...
              (EXIT
               (CONS (CONS (QCAR (|SPADfirst| |x|)) |u|)
                     (SPADCALL (CDR |x|) (CDR |y|) (QREFELT $ 60))))))))))) 

(DEFUN |IndexedDirectProductObject| (&REST #1=#:G328)
  (PROG ()
    (RETURN
     (PROG (#2=#:G329)
       (RETURN
...
(DEFUN |IndexedDirectProductObject;| (|#1| |#2|)
  (PROG (|pv$| #1=#:G324 #2=#:G325 #3=#:G326 #4=#:G327 $ |dv$| DV$2 DV$1)
    (RETURN
     (PROGN
        ...
        (PROGN
         (QSETREFV $ 20
                   (CONS #'|makeSpadConstant|
                         (LIST (|dispatchFunction| |IDPO;Zero;$;19|) $ 20)))
         (QSETREFV $ 42 (CONS (|dispatchFunction| |IDPO;zero?;$B;20|) $))
         (COND
          ((|domainEqual| |#2| (|NonNegativeInteger|))
           (PROGN
            (QSETREFV $ 45 (CONS (|dispatchFunction| |IDPO;+;3$;23|) $))))
          ('T (QSETREFV $ 45 (CONS (|dispatchFunction| |IDPO;+;3$;24|) $))))
         (QSETREFV $ 48 (CONS (|dispatchFunction| |IDPO;*;Pi2$;25|) $)))))
      (COND
       ((|testBitVector| |pv$| 1)
        (PROGN
         (QSETREFV $ 50 (CONS (|dispatchFunction| |IDPO;-;2$;26|) $))
         (QSETREFV $ 53 (CONS (|dispatchFunction| |IDPO;*;I2$;27|) $))
         (QSETREFV $ 55 (CONS (|dispatchFunction| |IDPO;-;3$;28|) $)))))
      (COND
       ((|testBitVector| |pv$| 5)
        (QSETREFV $ 58
                  (CONS (|dispatchFunction| |IDPO;subtractIfCan;2$U;29|) $))))
      (COND
       ((|testBitVector| |pv$| 3)
        (QSETREFV $ 59 (CONS (|dispatchFunction| |IDPO;<;2$B;30|) $))))
      (COND
       ((|testBitVector| |pv$| 2)
        (QSETREFV $ 60 (CONS (|dispatchFunction| |IDPO;sup;3$;31|) $))))
      $)))) 
...
(MAKEPROP '|IndexedDirectProductObject| '|infovec|
          (LIST
           '#(NIL NIL NIL NIL NIL NIL (|local| |#1|) (|local| |#2|) '|Rep|
              ...
              |IDPO;listOfTerms;$L;18| (105 . |zero?|) (110 . +)
              (116 . |zero?|) (121 . +) (|PositiveInteger|) (127 . *) (133 . *)
              (139 . -) (144 . -) (|Integer|) (149 . *) (155 . *) (161 . -)
              (167 . -) (|Union| $ '"failed") (173 . |subtractIfCan|)
              (179 . |subtractIfCan|) (185 . <) (191 . |sup|) (197 . |sup|)
              (|SingleInteger|) (|String|) (|HashState|)
              (|NonNegativeInteger|))
           '#(~= 203 |zero?| 209 |sup| 214 |subtractIfCan| 220 |smaller?| 226
              |sample| 232 |reductum| 236 |monomial| 241 |min| 247 |max| 253
              |map| 259 |listOfTerms| 265 |leadingTerm| 270 |leadingSupport|
              275 |leadingCoefficient| 280 |latex| 285 |hashUpdate!| 290 |hash|
              296 |constructOrdered| 301 |construct| 306 |coerce| 311 |Zero|
              316 >= 320 > 326 = 332 <= 338 < 344 - 350 + 361 * 367)
           'NIL
                  ...
                  (CONS
                   ...
                   (|makeByteWordVec2| 65
                                       '(2 7 9 0 0 10 2 6 9 0 0 11 1 7 13 0 14
                                         ...
                                         0 36 1 0 9 0 42 2 6 0 0 0 43 1 6 9 0
                                         44 2 0 0 0 0 45 2 6 0 46 0 47 2 0 0 46
                                         0 48 1 6 0 0 49 1 0 0 0 50 2 6 0 51 0
                                         52 2 0 0 51 0 53 2 6 0 0 0 54 2 0 0 0
                                         0 55 2 6 56 0 0 57 2 0 56 0 0 58 2 0 9
                                         0 0 59 2 0 0 0 0 60 2 6 0 0 0 61 2 11
                                         9 0 0 1 1 7 9 0 42 2 2 0 0 0 60 2 5 56
                                         0 0 58 2 9 9 0 0 31 0 7 0 1 1 0 0 0 26
                                         2 0 0 6 7 23 2 3 0 0 0 1 2 3 0 0 0 1 2
                                         0 0 24 0 25 1 0 37 0 41 1 0 34 0 40 1
                                         0 7 0 32 1 0 6 0 27 1 11 63 0 1 2 11
                                         64 64 0 1 1 11 62 0 1 1 0 0 37 39 1 0
                                         0 37 38 1 11 13 0 19 0 7 0 20 2 3 9 0
                                         0 1 2 3 9 0 0 1 2 11 9 0 0 12 2 3 9 0
                                         0 1 2 3 9 0 0 59 1 1 0 0 50 2 1 0 0 0
                                         55 2 7 0 0 0 45 2 1 0 51 0 53 2 7 0 46
                                         0 48 2 7 0 65 0 1)))))
           '|lookupComplete|)) 
