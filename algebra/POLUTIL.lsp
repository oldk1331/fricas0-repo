...
(DEFUN |POLUTIL;sylvesterSequence;2ThePolsL;2| (|p1| |p2| $)
  (PROG (|res| #1=#:G142 |term| #2=#:G141 |#G8| |#G7|)
    (RETURN
     (SEQ
        ...
        (SEQ (LETT |p1| (SPADCALL |p1| (QREFELT $ 21)) . #3#)
             (EXIT
              (LETT |res|
                    (PROGN
                     (LETT #2# NIL . #3#)
                     (SEQ (LETT |term| NIL . #3#) (LETT #1# |res| . #3#) G190
                          (COND
                           ((OR (ATOM #1#)
                          ...
                          (SEQ
                           (EXIT
                            (LETT #2#
                                  (CONS (SPADCALL |term| |p1| (QREFELT $ 22))
                                        #2#)
                                  . #3#)))
...
(DEFUN |POLUTIL;boundOfCauchy;ThePolsTheField;3| (|p| $)
  (PROG (#1=#:G145 #2=#:G144 #3=#:G146 #4=#:G151 |t| |l| #5=#:G150 |term|
         #6=#:G149 |c|)
    (RETURN
     (SEQ
      ...
      (LETT |l|
            (PROGN
             ...
             (SEQ (LETT |term| NIL . #7#)
                  ...
                  (SEQ
                   (EXIT
                    (LETT #6# (CONS (SPADCALL |c| |term| (QREFELT $ 27)) #6#)
                          . #7#)))
       ...
       (COND ((NULL |l|) (|spadConstant| $ 28))
             (#8='T
              (SPADCALL (|spadConstant| $ 28)
                        (PROGN
                         (LETT #1# NIL . #7#)
                         (SEQ (LETT |t| NIL . #7#) (LETT #4# |l| . #7#) G190
                              (COND
                               ((OR (ATOM #4#)
                              ...
                              (SEQ
                               (EXIT
                                (PROGN
                                 (LETT #3# (SPADCALL |t| (QREFELT $ 30)) . #7#)
                                 (COND
                                  (#1#
                                   (LETT #2# (SPADCALL #2# #3# (QREFELT $ 31))
                                         . #7#))
                                  ('T
                                   (PROGN
                                    (LETT #2# #3# . #7#)
...
(DEFUN |POLUTIL;lazyVariations;L2INni;5| (|l| |sl| |sh| $)
  (PROG (|s|)
    (RETURN
     (SEQ
      (COND
        ...
        (COND ((NULL |l|) (COND ((EQL |sl| |sh|) 0) (#1='T 1)))
              ((NULL (CDR |l|))
               (COND
                ...
                ((EQL |sl| |sh|)
                 (COND
                  ((EQL |sl| (SPADCALL (|SPADfirst| |l|) (QREFELT $ 36))) 0)
                  (#1# 2)))
                (#1# 1)))
              (#1#
               (SEQ
                (LETT |s|
                      (SPADCALL (SPADCALL |l| 2 (QREFELT $ 39)) (QREFELT $ 36))
                      |POLUTIL;lazyVariations;L2INni;5|)
                (EXIT
                 (+ (SPADCALL (LIST (|SPADfirst| |l|)) |sl| |s| (QREFELT $ 37))
                    (SPADCALL (CDR (CDR |l|)) |s| |sh|
                              (QREFELT $ 37))))))))))))) 

(DEFUN |RealPolynomialUtilitiesPackage| (&REST #1=#:G166)
  (PROG ()
    (RETURN
     (PROG (#2=#:G167)
       (RETURN
        (COND
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1 (APPLY (|function| |RealPolynomialUtilitiesPackage;|) #1#)
                (LETT #2# T . #3#))
            (COND
             ((NOT #2#)
...
(DEFUN |RealPolynomialUtilitiesPackage;| (|#1| |#2|)
  (PROG (|pv$| $ |dv$| DV$2 DV$1)
    (RETURN
     (PROGN
      (LETT DV$1 (|devaluate| |#1|) . #1=(|RealPolynomialUtilitiesPackage|))
      ...
      (COND
       ((|testBitVector| |pv$| 1)
        (PROGN
         ...
         (QSETREFV $ 37
                   (CONS (|dispatchFunction| |POLUTIL;lazyVariations;L2INni;5|)
                         $)))))
      $)))) 
