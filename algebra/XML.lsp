  ...
  (PROG (#1=#:G128 |line| |lines| |s1| |f1|)
    (RETURN
     (SEQ
      ...
      (LETT |s1| (SPADCALL |s1| (STR_ELT #3# 0) (QREFELT $ 12)) . #2#)
      (LETT |s1| (STRCONC |s1| "?>") . #2#) (SPADCALL |f1| |s1| (QREFELT $ 13))
...
(DEFUN |XML;writeVRML;XeSV;2| (|content| |filename| $)
  (PROG (|f1|)
    (RETURN
     (SEQ
      (LETT |f1|
         ...
         ('T
          (UNWIND-PROTECT
              (PROG1
                ...
                (LETT #1# T . #2#))
            (COND ((NOT #1#) (HREM |$ConstructorCache| '|ExportXml|))))))))))) 
