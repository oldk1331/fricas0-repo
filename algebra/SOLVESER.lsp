...
(DEFUN |SOLVESER;unvectorise;VFIF;1| (|vect| |var| |n| $)
  (PROG (|polyvar| #1=#:G128 |vecti| #2=#:G133 |i| Z)
    (RETURN
     (SEQ
...
(DEFUN |SOLVESER;decomposeFunc;4F;2| (|exprf| |exprg| |newH| $)
  (PROG (|newF| #1=#:G142 |newF2| |newF1| |m| N |degG| |degF| |g2| |g1| |f2|
         |f1| X)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |SOLVESER;subsSolve| (F |DegF| G1 G2 M HH $)
  (PROG (|resul| |solvevarlist| #1=#:G192 #2=#:G196 |solvar| |coeffma| |vec|
         |coeffmat| #3=#:G188 |i|)
    (RETURN
     (SEQ
      (EXIT
...
(DEFUN |TransSolvePackageService| (#1=#:G197)
  (PROG ()
    (RETURN
     (PROG (#2=#:G198)
       (RETURN
