...
(DEFUN |CPMATCH;makeComplex| (|p| $)
  (PROG (|rcoef| |icoef| |up|)
    (RETURN
     (SEQ
      (LETT |up| (SPADCALL |p| (QREFELT $ 14)) . #1=(|CPMATCH;makeComplex|))
      (EXIT
       (COND
...
(DEFUN |CPMATCH;makePoly| (|cs| $)
  (SPADCALL
   (SPADCALL (SPADCALL |cs| (QREFELT $ 24)) (QREFELT $ 12) (QREFELT $ 25))
   (SPADCALL (SPADCALL |cs| (QREFELT $ 26)) (QREFELT $ 27)) (QREFELT $ 28))) 

(DEFUN |CPMATCH;patternMatch;CSP2Pmr;3| (|cs| |pat| |result| $)
  (COND
   ((SPADCALL (SPADCALL |cs| (QREFELT $ 26)) (QREFELT $ 29))
    (SPADCALL (SPADCALL |cs| (QREFELT $ 24)) |pat| |result| (QREFELT $ 33)))
   ('T
    (SPADCALL (CONS (|function| |CPMATCH;makeComplex|) $)
              (SPADCALL (|CPMATCH;makePoly| |cs| $) |pat|
                        (SPADCALL (CONS (|function| |CPMATCH;makePoly|) $)
                                  |result| (QREFELT $ 37))
                        (QREFELT $ 39))
              (QREFELT $ 42))))) 
...
(DEFUN |ComplexPatternMatch| (&REST #1=#:G113)
  (PROG ()
    (RETURN
     (PROG (#2=#:G114)
       (RETURN
    ...
    (RETURN
     (PROGN
      ...
      (COND
       ((|testBitVector| |pv$| 1)
        (QSETREFV $ 43
                  (CONS (|dispatchFunction| |CPMATCH;patternMatch;CSP2Pmr;3|)
                        $))))
      $)))) 
