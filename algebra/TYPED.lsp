...
(DEFUN |TYPED;var;SIl$;2| (|n| |t| $) (CONS |n| |t|)) 

(PUT '|TYPED;getName;$S;3| '|SPADreplace| 'QCAR) 

(DEFUN |TYPED;getName;$S;3| (|v| $) (QCAR |v|)) 

(PUT '|TYPED;getType;$Il;4| '|SPADreplace| 'QCDR) 

(DEFUN |TYPED;getType;$Il;4| (|v| $) (QCDR |v|)) 

(DEFUN |TYPED;toString;$S;5| (|v| $)
  (PROG (|s|)
    (RETURN
     (SEQ
      (LETT |s|
            (SPADCALL
             (LIST (QCAR |v|) ":" (SPADCALL (QCDR |v|) (QREFELT $ 14)))
             (QREFELT $ 16))
            |TYPED;toString;$S;5|)
      (EXIT |s|))))) 

(DEFUN |TYPED;parseVarTerm;SNniR;6| (|t1| |pin| $)
  (PROG (#1=#:G125 |pt| |r| |ch| |vnm|)
    (RETURN
     (SEQ
      (EXIT
       (SEQ (LETT |vnm| "" . #2=(|TYPED;parseVarTerm;SNniR;6|))
            (LETT |pt| |pin| . #2#) (LETT |ch| (STR_ELT1 |t1| |pt|) . #2#)
            (SEQ G190 (COND ((NULL (SPADCALL |ch| (QREFELT $ 20))) (GO G191)))
                 (SEQ (LETT |vnm| (SPADCALL |vnm| |ch| (QREFELT $ 21)) . #2#)
                      (LETT |pt| (+ |pt| 1) . #2#)
                      (COND
  ...
  (PROG (|dv$| $ |pv$|)
    (RETURN
     (PROGN
      ...
      (|stuffDomainSlots| $)
      (SETF |pv$| (QREFELT $ 3))
      (QSETREFV $ 6 (|Record| (|:| |nme| (|String|)) (|:| |typ| (|ILogic|))))
      $)))) 
